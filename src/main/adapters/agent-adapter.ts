 ...
 * 消费方定义接口，生产者去满足它 —— 这是 §4.1「服务通过构造注入」的直接推论。
 *
 * ★ **M7a 更新：下面这段原文的论证有一半是错的，但结论活了下来。**
 * 原文写的是「**必须带结构化的 `messages[]`，绝不能是一个预先序列化好的 NDJSON blob**，
 * 因为 §4.6 整段论证的前提就是『历史以真正的消息数组传递』」。
 * M7a 探针实测推翻了那个前提：**写进 stdin 的多条消息不是「一轮的多条」**，
 * CLI 把 3 行当成 2 轮（判据是 `type:"result"` 的行数，
 * 见 `scripts/evidence/m7a-2026-09-23T16-51-08-216Z`）。
 * 所以**线上格式本来就只有一条 blob**，`renderTurnInput()` 那个拍平
 * **不是临时实现，就是正确答案**。
 *
 * `messages[]` 因此**保留**，但身份变了：它不再是「CLI 的输入格式」，
 * 而是**装配层与适配层之间的编码** —— 拍平只发生在最后一行代码上。
 * 保留它的收益与 prefix 缓存无关（缓存靠的是拼接的**纯追加性**，与几段无关），
 * 而是让「谁说的」在层间仍然是结构、不是文本。
 * 值由 `domain/context-builder.ts` 算（M7a 起），本文件只管形状。
 *
...
export interface TurnContext {
   ...
   * ★ **层间编码，不是线上格式。**（M7a 起）
   *
   * 由 `domain/context-builder.ts` 装配。适配器负责把它**拍平成一条文本**
   * （`renderTurnInput()`）再包成 stdin 的**恰好一行**（`userMessageLine()`）——
   * M7a 探针实测：发 N 行 = 发 N 轮，不是一轮的 N 条上下文（见文件头第 2 条）。
   */
