...
#### 3.1a 那条禁令的边界：assistant 的 `@` 从哪来（M7b 决策 2 的落地）

§3.1 禁的是**从自然语言里猜 `@` 指向谁**。它**不**禁止「由我们定义一块机器可读的格式、
让模型往里填」—— 后者的失败形态是「**没有那一块 = 没有派发**」，而不是「猜错」。

**用户侧：结构化采集，零解析。** `Composer` 选中成员即写入 `mentions: [{memberId, kind}]`，
`contentText` 里存**人名的字面文本**（用户看到的就是他打的那句）。

**agent 侧：`systemPrompt` 里逐字给出格式**（`context-builder.ts` 的 `COLLABORATION_PROTOCOL`）：
...
我们从这块**由我们定义、位置固定、无歧义**的块里解析。四条实现口径：

- **取最后一个块，不取第一个**：模型可能在正文里**引用**这个格式（解释它、举例），
  而协议规定真正的派发行在**最后**。取最后一个与协议同序；取第一个会把「举例说明」
  当成真派发 —— 那是一种**看起来正常**的错。
- **四种状态如实分开**：`none`（没写块 = 不派发，**这是正常结局**）、`ok`、
  `empty`（写了空块 = 不派发 + 一条诊断）、`malformed`（出现**孤立**的 `<mentions>` /
  `</mentions>` = 这一轮不派发任何成员 + 一条诊断）。「模型写坏了」与「模型没写」是两件事，都不猜。
- **中文逗号照切但不静默**：协议要求英文逗号，模型常写中文的；照样切开，同时记一条 note ——
  从结果上看不出这件事，不说就是静默失败。
- **孤立的 `<mentions>` 字面量不算块**：转述消息里那句说明**故意印着**这几个字（见 §4.5b-1），
  所以「正文里出现 `<mentions>` 就是坏块」是个**错的**判据，正确判据是「出现**完整的块**」。

**`kind` 取值收口**（schema 里是 `'to' | 'cc'`，语义由 M7b 定死）：

| `kind` | 含义 | 落地 |
|---|---|---|
| `to` | 派发一轮 | 写一条转述消息 + 建一轮（`hop_depth = 派发方 + 1`），**走去重** |
| `cc` | 抄送 | **只**写一条 `role: 'user'` 的历史行，**不建轮次**、**不去重** |

`cc` **不许**留在 schema 里不实现 —— 那会出现「界面上标了 cc、实际什么都没发生」的界面谎言。
它也不是装饰：被 cc 的成员**下一次开口时就会看到这条消息**，成本是一条 `message.append`、零轮次。
两种转述的措辞不同：`to` 是「【派发】…把这件事交给你。」，`cc` 是「【抄送】…你不需要行动，只是让你知道。」。

**历史正文一律原样带上**（模型看得见「@Nyx」这个词，那本来就是对话的一部分）。
「不许解析」说的是**派发方向**，不是显示方向 —— 用结构化的 `mentions` 去重写历史正文，
会让模型看到一份与用户看到的不同的对话。
**唯一被删掉那一行的地方是转述**：`<mentions>` 块是**我们与派发方之间**的机器指令，
转发给下游模型等于把「谁该被派发」这个已经做过的决定**再交给下游模型猜一次**，它会照抄。
...
**「哪几跳算同一条链」的判据（M7b 定死）**：链 = **本空间内**、按 `queued_at` 升序的轮次里，
`hop_depth` **逐跳 `-1` 的那一段后缀**（从刚结束的那一跳往回走，每一步要求深度恰好小 1，
不满足就停；往回看的上限 32 条）。展开在 §4.5b-1，这里只记两条**属于本节**的性质：

- **是「本空间内」，不是「本 session 内」** —— 跳数沿链**全局**递增，而 A↔B 的乒乓
  **天然跨两个会话**（每个成员一个），所以按 session 走那条后缀时两边都只剩一跳，
  熔断永远不可能触发。这是 §3.3 与 §4.5b 两条原文的**联合结论**，不是口味。
- **用户手动发起的一轮（深度 0）会打断递增** ⇒ 它要么是链的根、要么把链切开，
  新对话不会被误算进旧的链。判据**零新列、零内存状态、零 migration** ——
  于是走查的 `db-*.json` 转储里就有全部判据，**报告可以从归档重判**。
...
#### ★ 4.2-1 `inject_mode` / `compacted_through_seq` / `rolling_summary` 的**消费口径**（M7c 落地，2026-09-24）

这三列从 M0 就在 DDL 里，M7a 落地了**读侧**，而 M7c 是让 `compacted_through_seq > 0`
**在生产里第一次可达**的里程碑。也就是说：这条路径上任何「错了不报错」的形状，
都是在 M7c 才第一次生效的 —— 所以口径必须写下来，而不是留给下一个读者去猜。

**① 权威是 `session.compacted_through_seq`（一个数），不是逐条的 `inject_mode`。**
装配时按水位线**一刀切**（`seq <= 水位线` 的行不进数组，见 §4.6）。
逐条读 `inject_mode = 'summary'` 当过滤条件看起来「更精确」，但它把「哪一段被压了」
变成了**每一行都必须对**的状态 —— 而水位线只有一个写方、一个读方。
两个答案不一致时的表现是「有些被压的消息还进上下文」：**不报错，只是上下文比预期大**。

**② 逐条标记与 `summary_text` 是**诊断**，不是第二事实源。**
`markCompactedBySession` 把被折的行标成 `inject_mode='summary'` 并写上 `summary_text`，
用途是让「这一段被折过」在库里**肉眼可见**、让走查能数条数（M7c 的走查正是这么用的）。
同一段文本在 `session.rolling_summary` 里也有一份 —— **装配层从不读 `summary_text`**，
所以它过期、为空、甚至写错都不影响提示词，只影响诊断。★ 别把它当成「按条读」的入口。

**③ `excluded` 只读不写，且压缩不许覆盖它。**
装配层读 `inject_mode='excluded'`（把那些行排除掉），而**它的生产者至今不存在** ——
语义是「用户手动排除某条消息」（仍留在库里、仍在界面上，只是不进上下文）。
留着一个没有生产者的状态是刻意的：删掉唯一的读分支，那个已经写进 schema 的语义
就**永远不可达**了，而它是压缩替代不了的（压缩是自动的、成段的；排除是手动的、逐条的）。
> ⚠️ 反过来说：`excluded` 一旦要有生产者，必须**同时**给它一个可见的入口（§4.6a 的镜像问题：
> 计算了不渲染是缺陷；这里读了一个永远没人写的状态，是同一个缺口的另一面）。

**④ `markCompacted` 已改名 `markCompactedBySession`，粒度从 workspace 收窄成 session**（M7c）。
原来那份实现的 WHERE 是 `workspace_id = ?`，而水位线是**每会话**一个数 —— 按 workspace 接线的话，
压缩成员 A 的会话会把**同一空间里 B、C 的消息**一起标成 `summary`，而它们的水位线还是 0。
后果的形状值得记：库里出现「被标了但没被压」的行，而装配层只看水位线，于是它**照样工作** ——
这个破坏**不报错，只让库说谎**（还会让 ④ 的 `excluded` 保护在第二次压缩时失效）。
UPDATE 的谓词因此是：
...
- `inject_mode = 'full'`：保住「已被明确排除」的意图（否则压缩把它改写成 `summary`，
  抹掉一个装配层正在读的状态 —— 等于「被排除的内容借摘要回到上下文」，而它一声不响）。
- `deleted_at IS NULL`：软删除的行不参与。

★ 改名与改粒度的时机是**零生产调用方**的那一刻（M7c 之前它只有一条测试碰得到）——
这正是 §8.9-9 那条「一个零调用方的错误答案不会自己消失，它只是等着第一个调用方把它捡起来」
的正面用法：**在第一个调用方出现之前处置它，是最便宜的**。

> ★ **这条索引的分量在 M6b 落地时又被量了一次：聊天流的粒度是「空间」，不是「会话」。**
> 一条时间线里，用户消息与各成员的 assistant 消息**交错**排在一起，按 `message.seq` 全序；
> `message.session_id IS NULL` 就是「空间流里的用户消息」。所以
> **`message:list { workspaceId }`（不给 `sessionId`）拿到的正是整条空间时间线** ——
> 它不是一个「按会话过滤」的接口。会话只是**某一轮的执行归属**（哪次 CLI、哪条 session），
> 不是**阅读单位**：按 session 分桶会把一条时间线切成互不相干的几段，而用户读的是那条线。
> 连带地 §4.7 的 `order` 键从 `sessionId` 修正为 `workspaceId`（见 §4.7）。
>
> ⚠️ 这一条推翻过一份实现计划里「在 `App.tsx` 加一个 `selection.kind === 'session'` 分支」的说法 ——
> 那说法把「会话」当成了聊天流的粒度。写在这里，免得下一个人再顺着会话的形状去设计界面。
...
三条实测到的、容易写错的推论（前两条 M6a，第三条 M6b）：
...
- **★ 帧水位线**从历史接口里**推导不出来**（M6b 落地时才被真正踩到）。`message:list`
  只带**行 seq**（每消息）与**消息 seq**（每 workspace），而帧 seq 是**每 session** 的
  且不落库 —— 三条轴各说各的，历史行里没有一条能回答「我已经收到过这一 session 的哪一段帧」。
  这个数是**渲染层自己记下来的**，而且只在「它看着这一轮跑」的时候才存在。
  由它直接推出一条禁令：**「`message:list` 载入到哪儿，就拿那个 seq 当 `fromSeq`」是错的**。
  拿消息 seq 去撞帧 seq，只会有两种结果：撞上 `fromSeq > s.seq`，或者撞上
  `fromSeq < s.retainedFrom` —— 两条都回 `matched:false`，而 `matched:false` 在这里
  又**恰好长得像「这一轮没产出任何东西」**。M6b 的第一版协议就是照那句话写的，
  后果见下一节。
...
- 切换时，渲染层先 `message:list` 载入持久化历史，再 `stream:resume({sessionId, epoch, fromSeq})`
  要回在途帧并恢复实时推送。
  ★ **`fromSeq` 不是「已载入的最后一个 seq」**（那句话是本文档原来的写法，M6b 落地时被证伪，
  见上面第三条推论）。它只能来自**渲染层自己记的帧水位线**，
  而「要整轮还是要尾巴」由 `TurnWatch` 决定 —— 完整协议见 §4.3-3。
...
> ✅ **M6b 落地了**（`stream:resume` 已在 M6a 从 `defer` 转正，见 §4.3b；渲染侧的
> `epoch` / 水位线 / `TurnWatch` 全在 `src/shared/live/watermark.ts`，那份文件头把这段
> 论证又写了一遍，因为读那份代码的人多半不会翻到这里）。
> **epoch 是免费的正确性**：现在不加，M6 之后调试「切回来就不动了」会非常贵。

#### 4.3-3 ★ 重放协议（M6b 落地，含一处对本文档的**修正**）

§4.3-2 的第三条推论说了「帧水位线推不出来」，这一节说**那它从哪儿来**。
全部状态在两个纯函数模块里（`src/shared/live/watermark.ts`、`frame-buffer.ts`，裸 Node 可测）。

渲染层记**两样按轮次**的东西，外加一条按 session 的水位线：

| 记什么 | 作用域 | 它回答的问题 |
|---|---|---|
| `watermarks[sessionId]` = 已应用过的最大 `toSeq` | 每 session，**跨轮次** | 「这一 session 的帧我收到过到哪儿」 |
| `turns[sessionId].startSeq` | **每轮次** | 「要整轮，得报哪个数」（= 换轮那一刻的水位线） |
| `turns[sessionId].fromStart` | 每轮次，**只判定一次** | 「这一轮我是不是从头看着的」 |

★ **为什么必须按轮次多记一个 `startSeq`**：主进程的 `beginTurn` 在**新一轮的第一帧**到来时
把上一轮的在途帧丢掉（`retainedFrom = max(retainedFrom, s.seq)`）并清空回放环 ——
**环里永远只有当前这一轮**。于是拿跨轮次的水位线去问，只能要到**尾巴**；
要整轮必须报 `startSeq`（它恰好等于主进程那边的 `retainedFrom`）。
第一版协议就是拿跨轮次水位线去问的，拿回来的是**半截正文**。

★ **`fromStart` 为假时不许建缓冲**（判据：这一轮见到的**第一批**帧的 `fromSeq` 是否落在
`startSeq` 上）。那是「新进程打开一个已经在跑的轮次」：我们没有它的开头，
而**缓冲一旦存在就会抑制历史行** —— 半截缓冲会把完整的历史盖掉，
用户看到的是「回答只有最后一段」。退化成历史行是**正确**的：`content_text`
就是这一轮到目前为止的正文。代价是轮次结束前它不再增长（历史只在终态后重读），
这条已知且记在这里。

**有缓冲**（看着它跑、切走又切回）→ 要尾巴（水位线）。**没有缓冲** → 要整轮（`startSeq`）。
★ 「这个 session 没有水位线」**不是**不重放的理由：本会话于本进程的第一轮上 `retainedFrom`
还是 0，`fromSeq: 0` 恰好是对的答案，一次就能把整轮要回来。

**`matched: false` 的三种成因一律同解**（纪元不同 / 水位线超前 / 环里已淘汰）：
丢水位线 → 用响应里的 `epoch` 覆盖本地 → 整段重跑 `message:list`，**且不重试** ——
重试永远推不出正确的水位线。三种成因塌成同一个响应是刻意的，因为渲染层的动作完全相同。

**抑制与重放是一件事的两端**：`retain()` 在 `commit()` **之内**、`emitFrames` **之前**跑，
所以被抑制的帧**照样进了会话环** —— 之后那次 `stream:resume` 拿回来的就是**恰好那一段缺口**。
`stream:status` 永不抑制（按 workspace 抑制的是批次，不是状态）。

> **★ 终态那条路的代价（M6b 实测逼出来的）**：`endTurn` → `commit(turnId, 终态)`，
> 而 `commit` 在 `t.buf` 为空时返回 `[]`、`emitFrames` 在 `frames.length === 0` 时什么都不推。
> 于是**如果 `done` 帧已经在被抑制的窗口里刷出去了，`endTurn` 这一下不推任何批次** ——
> 渲染层唯一能知道「它结束了」的信号就是**迟 249–377ms 的 `stream:status`**（§4.3-2 量过）。
> 所以：**「流式结束」必须挂在终态 `stream:status` 上，不能挂在 `done` 帧上**。
> `done` 帧说的是「这一轮的正文说完了」，终态推送说的是「`turn` 行现在是终态」——
> 它们相差那 249ms，正好是**两个不同的事实**。
...
> **M5 的一处明知故犯 —— 2026-09-23 已由 M7a 探针结案（§8.9-19）**：
> M5 往 stdin 只写**一条** user 消息、由 `renderTurnInput()` 把数组拍平成文本，
> 当时登记为「暂时违反 §4.6」。
>
> **实测结论是：那个做法是对的，而它当时给的理由是错的。** 探针查了两件事：
> ① 内层 `message.role` 写 `assistant`/`system` ⇒ CLI 在**解析 stdin 阶段就把整轮拒掉**
> （stderr：`Expected message role 'user', got 'assistant'`，stdout 0 行）；
> ② 多条 user 行**不是**「一轮的多条消息」—— 3 行 → **2 条 `result`**，那个协议是**多轮对话**协议。
>
> ⇒ 「历史用数组」仍然成立，但它是**层间的编码**，不是线上格式：数组留在 `TurnContext.messages` 上，
> **拍平只发生在最后一行代码上**，而且现在**必须**拍平（有实测依据，不再是保守起见）。
> §4.6 那半段「单块 blob ⇒ 缓存永远命中不了」的论证也被同一次探针修正了（见 §4.6）。
...
#### 4.5b-1 三条规则的落地（M7b，2026-09-24）

上面三条是**规则**，这里记**判决在哪、执行在哪、以及哪一条做不到**。
纯判断全在 `src/main/domain/mention-service.ts`（零运行时依赖、**一个字节都不写库**），
执行全在 `src/main/process/fanout.ts`（唯一入口纪律见 §4.5a 规则一：派发只经过 `dispatch`，
取消只经过 `cancelQueued`）。**这一节的存在理由**：这三条从 M5 改派给 M7（见本节开头的归属更正），
而「机制在 A 节、触发条件在 B 里程碑」的文档最容易出的事就是**在 M7 被当成「已经有人做了」**。

| 规则 | 判定点 | 判决 | 落地 |
|---|---|---|---|
| ① 乒乓 | **扇出之前** | `pingPongOf(hops)` → `none` / `warn` / `terminate` | 终止分支**刻意不调** `fanoutOf`（终止＝本轮不建新轮次）；警告分支**照常派发** |
| ② 去重 | **扇出时** | `fanoutOf()`，命中则出一条 `mention-deduped` | 被去重的目标**既不建轮次、也不写转述行** |
| ③ 广播 | —— | **没有判决函数** | 一条注释（`mention-service.ts` 文件头）+ 走查一条断言 |

**① 的阈值是两个精确值，不是「≥ 就报」**：`emptyHops === 2` 才警告
（用 `≥ 2` 会让第 3 跳再弹一遍同样的话，那是噪音）；`emptyHops >= 4` 才终止
（用 `≥` 是因为**终止必须兜住** —— 万一某一路没能终止，下一跳不能就这么放过去）。
「任一跳有实质工作就清零计数」**不需要可变计数器**：从链尾往回数、数到第一个有工作的跳为止，
数出来的就是那个计数器的值（`pingPongOf` 里就是一个 `for`，因此没有「计数器忘了重置」这种状态可丢）。

**★ 「实质工作」的判据只有一处：`WORK_EVENT_KINDS = ['tool_start', 'file_diff']`。**
第三条「**没有新信息**」**判不了** —— 那需要一个语义比较器，我们没有，而装作有会得到一个
**看起来正常**的判决，那正是本项目最贵的那类 bug。后果如实记，且**方向要说清**：
一次纯粹的信息交换（模型真在推理、但没碰工具）会被算成**空转**，于是链可能在 4 跳处被终止。
这是**偏严**的方向，与本条「宁可漏杀，不可错杀」**相反** —— 属于明知的取舍，写下来，不藏。
`tool_result` 不算（它总跟着一个 `tool_start`，算上只会重复计数），`text` / `thinking` 不算
（那是「说了话」，正是本条要判的空转）。

**★ 链的判据是「跨 session」的（这一点修正了 M7 计划书 §5.2 的措辞）。**
计划书写的是「本 session 内」，但 §3.3 定死了「派给 B 的那一轮 `hop_depth = A.hop_depth + 1`」——
跳数是**沿链全局**递增的；而 A↔B 的乒乓**天然跨两个会话**，于是 A 的会话里只有 0、2、4 跳、
B 的会话里只有 1、3、5 跳：**按 session 走那条「逐跳 +1」的后缀，两边都只有一跳，
熔断永远不可能触发。** 判据因此是「本空间内、按 `queued_at` 升序」的后缀。四条实现细节：

- **起点是「刚结束的那一跳」，不是「最近的那一条」** —— 并发下会有轮次乱序收尾（先入队的后结束），
  按「最近一条」会把链尾错接到另一个会话的轮次上，而**那种错不报错**。
- `queued` / `running` 对判链**不可见**：既不算一跳（实质工作还没落定），也**不占位**
  （否则一条正在排队的兄弟轮次会把链切开）。**代价是这一步会漏**，三种形状已知且都不报错：
  ① **同一次扇出的兄弟跳**（同深度）后收尾的那几条：它往上接不到「深度恰好小 1」的前一跳
  （下一跳是**同深度**的另一个兄弟）⇒ 后缀断在它那里，链只剩一跳；
  ② **发言者那一跳还在跑**时兄弟跳先收尾：同样接不上（那一跳对判链不可见）；
  ③ 同空间里另一条并发 @ 链交错时，链上某两跳在 `queued_at` 序里被隔开。
  三者都是「宁可漏杀」的直接代价 —— **① ② 在第三次真跑里都实际发生了**（见下面后果 1）。
- 往回看的上限是 `CHAIN_TAIL_LIMIT = 32` 条；起点不在窗口内时**按「这一跳单独成链」**处理，
  并报一条 `fanout:chain-tail-missing`（判不出空转 ⇒ **不终止**，方向仍是漏杀）。
- **记录被否决的方案**：内存里维护链状态更省查询（照 `scheduler.ts` 的 `busySessions` 先例），
  但进程重启后计数从 0 开始 —— 会让「一个安静的丢失」成为可能。用库里的 `hop_depth` 则
  **报告可以从归档重判**（§8.8e 纪律）。

**★ 「强制终止该链」= 取消扇出 + 取消排队，不是杀进程。** 终止分支里**刻意不调 `fanoutOf`**；
被取消的是**能被链上某一跳认领的**排队跳 —— 认领要**同时**满足两条：`hopDepth` 恰好等于
链上某一跳 `+1`，**且**它的触发消息的作者就是那一跳的成员（转述消息的作者 = 派发方，
这是全仓库唯一能从库里读出「这条排队跳是谁派的」的线索）。少了后半条，同空间另一条并发 @ 链的
排队跳会被一起取消 —— 那是**错杀**，而本节的口径是宁可漏杀。
`hopDepth === 0`（用户直接发起）**永远不在取消之列**：他不是链的一部分，取消他那一轮等于替他做决定。
**已知仍会误伤的一种情形**（写下来）：另一条链的派发方**恰好**是链上某一跳的同一个成员、
且深度也对得上时会被认领 —— 需要两个巧合同时成立。
落地三步**照抄 `turn:stop`**：先 `markCancelled`（库，包在 `store.tx()` 里）→ 再 `cancelQueued`
（内存队列）→ 再 `emitStatus`。那个顺序在 `handlers/turn.ts` 里有逐字论证，**别改**。
**不新增 `runtime.cancelChain`** —— 用现有两个原语组合。
正在跑的那一轮**不 abort** —— 它的产物是合法的（它确实是那条链上的那一跳，只是之后不该再有）；
运行中轮次的中断是 M9 的活（§4.4b）。

**落库的那条 system 事件落进「刚结束的那一跳的会话」**，不落空间流（`session_id = NULL`）——
乒乓是链的局部现象，写进空间流会让别的成员也看到。跨会话的链**没有「链所在的会话」**，
这一点**修正了计划书 §5.3 的措辞**。它**刻意不带 `turnId`**（这条行是事后追加的，
不属于那一轮的产物 —— 带上就会被 `listByTurn` 当成那一轮说的话），
且**正文里一个跳数数字都不出现**（§3.3：跳数绝不进提示词），数字只进 `notes` / 日志 / notice 的 `detail`。
`app:notice` 与日志各带一份 `chain`（逐跳的 `{hopDepth, hadWork}`）与 `cancelled`（被取消的轮次 id），
**报告因此能从归档重判这一判决对不对**。

**「不加全局 hop 上限」是决定，不是遗漏**：`hop_depth` 只在界面与状态上显示，
上限靠 ①② 与并发槽位。第 3 条已经明文拒绝第四个机制，`mention-service.ts` 文件头里
那段抄写就是用来**拦住下一个人**的。

**走查（第三次真跑，`scripts/evidence/m7b-2026-09-23T18-01-02-681Z`）看到的三件后果**：

1. **2 跳警告在「一条普通的单 `@`」上就可能触发 —— 也可能整条链一次都不触发，取决于谁先收尾。**
   空转计数**从链尾往回数**，0 跳那一跳（用户发起、由发言者执行）**也在计数里** ——
   只要发言者那一轮自己也没碰工具，它 @ 出去的那一轮结束时 `emptyHops` 就是 2，警告当场弹出。
   第三次真跑的 Rill 链正是这个形状（`Vex@0` 与 `Wisp@1` 都没碰工具 ⇒ 警告落在 1 跳上，
   归档里的 `fanout:ping-pong-warn` 带 `emptyHops: 2` 与那两跳）。
   ★ **但同一个归档里的 Sable 链给出了反例**：它每一跳都没有实际工作
   （**整个归档的 `tool_start` / `file_diff` 事件数是 0**），却**全程一条警告都没有**。
   原因见上面「不可见」那一条的 ① ②：`Nyx@1` 先收尾时，发言者 `Atlas@0` **还在跑**
   （对判链不可见）；`Echo@1`、`Juno@1` 收尾时，往上一条是**同深度**的兄弟 ⇒ 后缀断在它们那里，
   链只剩一跳 ⇒ `emptyHops = 1` ⇒ 不警告。**两种形状都在这一次真跑里发生了。**
   ⇒ 这是「已执行过的跳才算跳」的直接后果，**方向是漏杀**（与本条第 1 款的「宁可漏杀」一致），
   **不是缺陷**；但它意味着**警告不能当作「这条链有没有在空转」的可靠读数**，只能当提示。
   要不要把 0 跳排除出计数（口径会变成「**派发之后**连续两跳没有动作」），**见 §8.9-20 的待办**。
2. **协作协议住在 `systemPrompt` 里、对人设中立** ⇒ 它**不区分工作空间**，
   于是**任何**空间都可能自发长出链（实测里 Sable 出现了 `Nyx@2 → Atlas@2`，与那条被设计出来的
   Rill 链并行）。按「空间级」读走查的数字会被它污染 —— 这是「协议是全局的」这件事的**实测后果**。
3. **转述正文逐字带着用户原文里的 `@Nyx @Echo @Juno`**（转述的用意就是让收件人看见原文），
   于是成员会照着那几个名字回 @ —— 实测观察到一条 `mention-self`（被规则 ④ 拦下，没有派发）。
   另外，转述的**那句说明里印着字面的 `<mentions>`**（它要告诉对方「你原文末尾那一行已由本应用取出」），
   所以「转述正文里不许出现 `<mentions>`」是个**错的**断言，正确判据是「不许出现**完整的块**」。

**去重是「内容同一性」的**：键的两半是 `(目标成员, sha256(转述正文))`。
⇒ 同一件事**换一种说法**再派一次**不会被合并**（它们是两条不同的正文），
只有**逐字节相同**的正文才会命中。副作用：去重键算不出来时（排队轮次的内容哈希取不到）
**不进基准**并报一条 `fanout:dedupe-basis-skipped` —— 后果是「少判一次重复」（多派一轮，看得见），
而不是「误判成重复」（少派一轮，安静的）。
**重启后基准是空的**：`queued` 的行在进程启动时被 `reapOrphans` 翻成 `failed`（§8.9-10），
于是去重**是进程内有效的、不是持久的**。这是正确且无害的（重启后本来就没有队列，§4.5「不自动恢复」），
但它必须记在**已知有损**里。

**一条实测的边界**：「终止时取消排队中的同链条轮次」在**线性链**下不可达 ——
链上每次只有一跳在跑，它后面没有兄弟排队跳，所以走查里这一条是 ⚠️ 而**不是 ✅**；
它由 `test/process/fanout.test.ts` 覆盖，**不靠真机**。写在这里是为了下一个读走查报告的人
不会以为「⚠️ = 坏了」。
...
**user 侧 —— 层间是**一个数组**，最后拍平成**恰好一条** user 行**（M7a 实测后定形，2026-09-23）

> ★ 原文说「历史以**真正的消息数组**传递，不是单块 blob」，理由写成「prefix 缓存以消息为界」。
> **M7a 的探针把这条推翻了一半**（逐字见 §4.4e 与 `scripts/m7a-probe.ts` 文件头）：
>
> 1. **stdin 内层 `message.role` 只能是 `'user'`。** 写 `assistant` / `system` →
>    CLI 在**解析 stdin 阶段就把整轮拒掉**（stderr 逐字 `Expected message role 'user', got 'assistant'`，
>    stdout **0 行**）。
> 2. **多条 user 行不是「一轮的多条消息」。** 3 行 → **2 条 `result`**，切在 `1 | {2,3}` ——
>    那个协议是**多轮对话**协议：发 N 行 = 发 N 轮。
>
> ⇒ 所以数组**留在层间**（`TurnContext.messages`），到适配器最后一步才由 `renderTurnInput()`
> 拍平、`userMessageLine()` 包成**恰好一行**写出去。数组是对的内部编码，但它**不是线上格式**。
> 数组里保留**逐条**的角色标记（`【我】`/`【用户】`/`【系统】`/`【名字】`，由 `historyLabelOf` 生成），
> 于是模型看得见「谁在说话」；**拍平只发生在最后一行代码上**。

**缓存前缀为什么仍然成立**（这一条是对原文那半段论证的**修正**）：
`renderTurnInput()` 是 `map().join('\n\n')` —— 一个**纯追加**的拼接。于是第 N 轮的输出**逐字节是**
第 N+1 轮提示词的前缀。原文那句「单块 blob 方案 ⇒ 前缀每轮都在变 ⇒ 缓存永远命中不了」
把**编码**和**内容策略**混成了一件事：破坏缓存的是**重写前缀里的内容**（比如每轮都变的 git 状态），
**不是「拼成一块」**。两条性质都有单测钉着。

**`<recent>` 不是一个块**（M7a）。窗口由 `session.compacted_through_seq`（**水位线**）切出来：
取 `seq > 水位线`、且在本轮**触发消息之前**的历史，条数另有上限 `historyLimit`（默认 200）。
它没有自己的标签、也没有自己的边界 —— 因为「最近的若干条」不是一个块，是**一个区间**。
把它当成块，就会让「窗口」和「水位线」变成两个都说了算的东西（§4.5a 规则一）。

**`<project_context>`（§8.5c）**：对本轮 **agent 可见的每个项目**跑一次
`AgentAdapter.collectProjectContext(rootPath)`，把结果按 `kind` + `path` 标注拼进**首条** user 消息。
它同时承担「告诉 llm 空间里有哪些项目、我能看到哪些」的职责 —— **不依赖文件系统遍历，不依赖 cwd**。
只出现在首条消息，所以**不影响前缀稳定性**。
★ `cwd` 的那一份 `CLAUDE.md` **被剔掉**（`AUTO_INJECTED_FROM_CWD`），因为 CLI 自己会带进去 ——
两个路径都在表里（`CLAUDE.md` 与 `.claude/CLAUDE.md`），依据见 §8.5c-1 与 `context-builder.ts` 头部。

**`<env>` 不含 git 状态**（M7a 的三条理由，只放 cwd / 项目名与根路径 / 可见项目清单）：

1. **它每轮都变** ⇒ 进首条 user 消息就等于**每轮重写前缀**，缓存全灭（§3.4 那个「30 轮 15.5×」要全额承受）。
2. 分支名与脏文件对「该改哪一行」帮助有限，而**代价是每轮一次子进程**（§8.5a）。
3. `<project_context>` 已经承担了「我手里有什么」的职责 —— 两处都答一遍，就会有两份**会分叉**的答案。

> 三条里只有第 ① 条是硬约束，另两条是取舍 —— 但它们指向同一个方向，所以不必再议。
> cwd 的**变化**仍然要告诉模型：走 §5.6 的 `message(role='system')` 行追加，不走重写 `<env>`。

`<env>` 正是 `--exclude-dynamic-system-prompt-sections` 设计用来从 system prompt 挪出去的东西 ——
所以**必须用 `--append-system-prompt`，绝不用 `--system-prompt`**。CLI help 明确写了该旗标
「只在默认 system prompt 下生效（`--system-prompt` 下被忽略）」。
...
> ⚠️ **M7c 的实测给这一条加了一个限定语**：本机默认凭据所指向的那个端点上，
> M7a（2 轮）与 M7c（3 轮）**五次请求的 `cacheRead` 与 `cacheCreation` 全部是 0**
> —— 但归因是「这个端点上这个字段不是『本请求命中了自己上一条前缀』那个意思」，
> 四条证据与「别拿 37248 当基线」的警告见 §5.4 与 §8.9-16。
> ⇒ 上面那套前缀论证**目前只有单测支撑**（「第 N 轮的输出逐字节是第 N+1 轮的前缀」那条性质
> 有单测钉着），真机侧在本端点换不到正例；别把这个 0 读成 bug，也别读成通过。

#### ★ 4.6-1 `<summary>` 的生成：**确定性摘要，不调模型**（M7c，2026-09-24）

**判决与执行分居两处**，照 §4.5a 规则一：

| | 在哪 | 性质 |
|---|---|---|
| 判决 + 文本生成 | `domain/compaction-service.ts` | **纯函数、零 runtime import**，所以「该压吗」「摘要长什么样」可以被单测穷举 |
| 执行（读库、写库、发通知） | `process/compaction.ts` | 挂在既有的 `onTurnFinished` 缝上，**排在扇出之前**（顺序见 §6.5.1） |

**触发条件是两个数**（都是**估算**，不是 token 计数 —— §5.3a 明说真实窗口未知）：

| 常量 | 默认 | 含义 |
|---|---|---|
| `compactAtCount` | **20**（`>=` 即触发，不是「超过」） | 「本轮之前」的历史条数 |
| `compactAtChars` | **400 000** | 那些历史正文的**字符数** |
| `summaryMaxChars` | 4 000 | 摘要文本的上限 |
| `ENTRY_MAX_CHARS` | 200 | 单条骨架的正文上限 |
| `COMPACT_READ_LIMIT` | 500 | 一次最多读多少条来折；**撞上限不假装压完了**（见下） |

> ★★ **`compactAtChars` 数的是「正文文本字符」，不是「骨架字符」。** 这一条是 M7c 写代码时
> 抓到的产品级后果：骨架每条被截到 200 字、读取上限 500 条 ⇒ 用骨架算的话这个闸的
> 可达上限只有约 100k 字符，**400k 这个阈值永远够不到** —— 一个永远不会触发的阈值
> 看起来和「阈值定得高」一模一样，它不报错。单测里有一条用例专门钉着「字符硬闸单独触发」。

**区间右端只有一个所有者**：`message-repo.lastSeqBefore(sessionId, triggerSeq)`
（`seq < ?` 且未删的最大 seq）。纯函数拿到的 `throughSeq` 是**回显**，它**不重算** ——
重算就等于「哪一段被压了」有了第二个答案，而两者的分歧不会报错，只会让某一段历史
既没进数组也没进摘要（或者进去两次）。

**摘要的形态**（`appendToRollingSummary`，四条纪律都写在文件里）：

```
【已折叠的历史摘要】seq ≤ 5 的历史已折叠成下面这段 5 条骨架（每行：[seq] 说话人 正文首段），原文不在你的上下文里。
[1] 【我】 用户第 1 轮的开头一段…
[2] 【我】 收到。
[3] 【系统】 …
```

1. **必须累积。** 上一轮的 `rolling_summary` **逐字携带** —— 否则第二次压缩会把最早那段历史
   静默吃掉（水位线一路前进，而摘要里没有它。**不报错**）。
2. 携带时**按本模块自己拥有的严格匹配**剥掉旧首行标题（前缀 + 两段切片 + 数字校验 + 尾后无余，
   **不用正则** —— 标题里有 `[` `]`，手写转义是又一个会静默失配的地方）。
   剥不掉就**逐字携带**并记一条 `warn`：这条降级路径只损失一个标签，**绝不丢字**。
3. 超上限时**从中间往外删、头尾都保**（开头是最早的历史，结尾是最近的；中间那段最容易被
   后来的摘要覆盖），交替从两侧各删一行，并在删除处插一行**带内声明**：
   `…（此处省略了 N 条较早的骨架 —— 原文不在你的上下文里，它们也不在本摘要里）…`
4. **声明行永远不许被删**（上限小到荒谬时输出「标题 + 声明」）。

> ★ 这四条合起来的口径是：**丢失可以发生，但必须自己说出来。**
> 而原文**仍在库里**（压缩只改 `inject_mode`，从不删数据，见 §4.2-1）——
> 所以这种丢失是**模型视角的**，不是真的丢了。§14.4 照这个口径记。

**确定性**：同一份输入跑两次**逐字节相同**（有单测钉着）。于是「那一轮为什么压成这样」
可以**复算** —— 不需要日志、不需要重放模型。这是选确定性摘要而不是「让模型总结」的主要理由
（另一个是：模型总结会把推理成本塞进折叠路径，而折叠正是上下文吃紧的时候）。

**可见性与通知**：压缩成功时写一条 `role:'system'` 的折叠说明行，**且不带 `turnId`**
（带上就会被 `listByTurn` 算成那一轮的产物）；另有一条 `warn` 日志（`compaction:compacted`）。
`app:notice` **只在 CLI 自己的压缩信号出现时才发**（见 §8.9-13 / §8.9-14 的露面表）——
本级压缩是**设计行为**，不该每压一次就给用户一条警告。

**阈值旋钮**：`CODE_CHAT_COMPACTION_N`（只覆盖 `compactAtCount`）。它**只被读进我们自己的进程**，
一个字节都不到子进程那边去（§4.4 的 `spawn` 那一行不用改）；值非法（`''`/`'abc'`/`'2.5'`/`'0'`/`'-3'`）
**先记一条 `[compaction] …` warn 再用默认值** —— 静默回退的坏处很具体：走查会得到一份
「什么都没压」的正常报告，而真正的原因（旋钮打错了）在任何地方都看不到。

**压缩轮必然一次全量缓存失配**：折叠把那一段历史换成了摘要块 ⇒ 那一轮的前缀**必然**变了。
这是**一次性**代价，下一轮起前缀重新稳定。★ 而这条论证在本端点**没有被观测到**
（`cacheRead` 五次全 0，见 §5.4）—— 它目前是机制推理，不是实测。
...
> ★ **M6b 落地：这一条最贵的一次落地在成本上**（2026-09-23）。§5.4 要求「UI 常驻显示累计成本」，
> 而累计值只能来自**聚合查询**（`turn:list` 带 `limit`，拿有上限的列表求和会**安静地变小**）。
> 聚合写在 `turn-repo.ts`，落点是 §5.4 的那个 `workspace:usage`。
>
> ⚠️ 关键的一处是**聚合本身也要遵守规则二**：`cost_usd` / `tokens_in` / `tokens_out` 都是**可空列**
> （一轮失败、被中断、进程被杀时它们就是 `NULL`），而 `SUM` **跳过 `NULL`** ——
> 于是 `SUM(cost_usd)` 会安静地给出一个**偏低**的数字，用户无从知道它偏低。
> 所以聚合**同时返回** `turnsWithoutUsage`（`SUM(cost_usd IS NULL)`），界面在它非零时如实附一句。
> **不留这个字段，那个金额就是在说谎** —— 这正是规则二反过来读：空缺不许冒充成 0。
>
> M6b 逐字段过了一遍「要么在界面上、要么在注释里写明为什么故意不显示」：
> 三个 token 字段上了顶栏（`title` 里带「入 N / 出 N token」），`turnsWithoutUsage` 有条件地
> 变成一句警告，`terminal_reason` 落在 `turn:get`/历史的诊断行里。

> ★★ **M7c 落地：这条纪律抓到的两次「算出来了但方向是错的」**（2026-09-24）。
> 两次都不是「忘了传」，而是**传了一个读起来像另一件事的数** —— 比不传更难发现。
>
> **① `<summary>` 那句里的条数会撒谎。** 它原来数的是**本窗口内**被挡掉的条数
> （窗口上限 `historyLimit = 200`），于是长会话下 500 条会被说成 200 条 ——
> 而且与摘要正文里那句**准确的**「共 500 条」**同场矛盾**。
> 修法是**不报数**：改成 `seq ≤ N 的历史已被压缩成下面这段，原文不在你的上下文里`，
> 让准确条数由摘要正文自报。**一件事只有一个所有者**，哪怕它只是一个数字。
> 这个 bug 只在 `through > 0` 时才可达 —— 而 `through > 0` 正是 M7c 才第一次让它可达的，
> 所以它躺了一整个里程碑没被发现。
>
> **② `ContextShape` 加了 `summaryChars`。** 在它之前，「`<summary>` 块到底有没有进数组」
> 只能**推断**（`compactedThroughSeq > 0`），而推断在「水位线推了、块被跳过」时**不报错**：
> 那正是本纪律要消灭的形状。`summaryChars` 是那个块的**直接**观测（没注入 = 0），
> 走查的判据从「应该在了」变成「在了，贡献了 149 字」。
> ★ 同一个教训的另一面：**`historyTotal` / `historyIncluded` / `historySuppressed` 三个数
> 是给 A/B 对照用的**，单独读会骗人 —— 压缩一次同时**删掉 2 行、加进 1 行** system 行，
> 净变化与「变小」不是一回事。`test/ipc/turn.test.ts` 里那条端到端用例因此配了一个
> **对照组场景**（默认阈值，不压缩），两个场景的 `historyIncluded` 才有可比性。

### 4.7 渲染层状态管理

**Zustand 5** + 三个 slice：`entity`（规范化实体）、`live`（在途缓冲 `buffers[turnId]`、空间时间线 `messages` + `order[workspaceId]`、水位线/纪元、运行概要、成本）、`ui`（活跃空间/会话、未读）。
...
>
> ✅ **M6b 兑现了它**：`live` slice 建起来了（`store/live.ts`），运行概要按原计划从 `ui` 迁到这里，`store/index.ts` 那句寄放注释随之作废。

★ **本节的 `order[sessionId]` 已修正为 `order[workspaceId]`**（M6b 落地时改的，见下面那条「`order` 的键」）。原因不是实现方便，而是**聊天流是空间级的**：`message.seq` 的 `UNIQUE (workspace_id, seq)`（§4.2）就是为「单一空间时间线」建的，用户消息与各成员的 assistant 消息交错在同一条线上。按 session 分桶会把一条时间线切碎成互不相干的几段。
...
**M6b 落地时新增的所有者**（`live` slice 与渲染层那一侧）：

| 事实 | 唯一所有者 | 谁引用、**谁不许复制** |
|---|---|---|
| **`order[workspaceId]`** / `messages[id]` | 主进程 `message-repo`（**DB 是事实源**） | 渲染层是**投影**：由 `message:list` 载入、终态折叠时补一条。★ 键是 **workspaceId**，不是 sessionId（见上文那条修正） |
| **`buffers[turnId]`** | 渲染层，由 `@shared/live/frame-buffer.ts` 的**纯函数**写入 | 唯一的写入口是 `applyBatch` / `resumeWorkspace`（`live.ts` 里没有第三个）。它同时是「抑制历史行」那件事的依据，所以**它的存在与否是有语义的**，不是一个缓存 |
| **帧水位线**（`watermarks[sessionId]`） | 渲染层（**本轮新增的唯一一个所有者**） | 值是「已应用过的最大 `toSeq`」；纪元一变整体作废。★ 它**推不出来**（帧 seq 不落库，§4.3-2），只能由「我收到过什么」记下来 |
| **`epoch`** | 主进程 `event-batcher`（每进程铸造一次） | 渲染层只**持有**它、并在 `stream:resume` 时回传。★ **不许自己生成** —— 编一个的后果是一次**看起来成功**的空重放（§4.3-2） |
| **`recentlyFinished`** | 渲染层 `live.ts` | 挡住「轮次已结束、帧还在路上」那一批：收下它们会重建出一个**只有尾巴**的缓冲，盖住一条正文已经完整的历史行 |
| **累计成本 / 累计 token** | 主进程 `turn-repo` 的聚合读（`workspace:usage`） | ★ 界面**只能**显示这个聚合值，不许拿 `turn:list`（**带 `limit`**）自己求和 —— 那种求和看起来完全正常，只在历史变长之后悄悄变小（§5.4） |

★ 同一张表里还有一条**接口**级的所有权，它不落在字段上却同样要指出所有者：`stream:status`
是 `runtime:getState` 的**唯一变更信号**（`PUSH_CHANNELS` 刻意只有四条，没有 `runtime:changed`）。
收到任何一条 `stream:status` 就重取一次运行概要 —— 这不需要新通道，而且两个用途
（轮次状态显示、队列深度刷新）**恰好同步**：轮次状态变了的时刻，正是槽位与队列会变的时刻。
...
```
store/live.ts                            ★ 第三个 slice：buffers / 时间线 / 水位线 / 成本
hooks/usePushNotices.ts                  ★ 改成订阅**全部四条**推送（M4 时只订了两条）
components/conversation/                 ★ 新目录，十个文件
  ConversationPane  MessageList  MessageRow  StreamingRow
  StreamingText  ThinkingPanel  ToolTimeline  DiffBlock  ActorChip  Composer
```

- **删掉 `components/mock/ConversationMock.tsx`（194 行硬编码假数据）。** 真对话看得见了，
  它的视觉语汇由 `components/conversation/` 继承。留着是最糟的那种东西：
  一份**看起来像真的**假数据，摆在一个真的旁边。`WorkspaceOverview` 里那个位置换成「去对话」入口。
- **纯逻辑搬到 `src/shared/live/`**（`frame-buffer` / `watermark` / `history` / `timeline` / `patch-lines`），
  零 DOM、零 electron，由裸 Node 直接测 —— 照 `copy-policy.ts` 的先例。判据是
  「**它是判断，不是渲染**」：留在组件里就只能靠「点开应用看一眼」验证，而它错起来是**静默**的
  （正文出现两遍、开头缺一段）。M6b 那个 epoch 缺陷正是这样被抓出来的（§六「M6b 实证」）。
- ★ **`ui.view`（概览 / 对话）与 `ui.selection`（在看哪个对象）互斥**，两个 setter 各让一步：
  `setView()` 清 `selection`（点「概览」不该跳进上一次点开的那个成员），
  `select(非空)` 顺手切回 `overview`（在对话视图里点侧栏成员**必须**有反应，否则是个静默的空点）。
  这两句必须同时存在 —— 只写一边，另一边就是「点击了什么都没发生」。
  判据放在 store 而不是两个调用点（`MemberList` / `ProjectList`）：**一个判据只写一遍**。
- ★ 「对话」是**空间级**的一屏，与 `selection` 并列而不是它的一个分支，
  所以它的开关在 `TopBar`（空间级的工具条）里，不在成员/项目详情面板里。**切空间不清 `view`**：
  用户以 Atlas 的身份聊到一半、去 Nuvola 看一眼再回来，期待的是回到对话，不是被丢回概览。
...
1. `<MessageList>` **只订阅 `order[workspaceId]`**。该数组除非**增删消息**否则引用不变 → 列表在 token delta 时**完全不重渲染**
...
> **M6b 落地时对规则 3 收得更紧了一点**（实现收紧，不是偏离）：`<StreamingRow>` **自己也不随
> delta 重渲染**。它订阅的四个值（`actorId` / `items` / `usage` / `done`）全是**低频且原子**的，
> 正文与思考**不在这个列表里** —— 它们各自在自己的组件里订阅。一次 delta 于是只重渲染
> **那一个 `<p>`**，而不是整张卡片（头像、时间线、diff、用量行）。
> 与原文那个版本唯一还共享的是「一个，不是 N 个」：两版都把整张 `<MessageList>` 挡在 30Hz 之外。
>
> 规则 7 的落地形态也值得记一笔：**折叠发生在 `stream:status` 的终态推送那一支，而不在 `done` 帧**。
> 两者相差 249–377ms（M6a 实测），而库里那一行是**后**落定的那一个 —— 在 `done` 时折叠会让界面
> 先闪回一条还没有正文的历史行。执行顺序是硬的：**先把历史读回来，再丢缓冲**（`live.ts` 里有长注释）。
...
> **M6b 的两处偏离（都记在这里，因为它们是「文档说要有、而实现里没有」的那种）**
>
> 1. ★ **不装 Shiki**，代码块走**等宽**。上面那句「语法高亮用 Shiki 且只在 `done` 时跑」**没有落地**。
>    M6b 的验收是「看得见流式输出」，不是「高亮好看」；不装 Shiki 就零新依赖。
>    ⚠️ 这是**偏离文档**，不是文档没写 —— 不记下来的话，下一个读这一节的人会以为高亮已经有了。
>    对照 §9 的「已知不做」：同样口径的一条是「流式期间不解析 Markdown」。
> 2. **光环不旋转**。技巧 3 的后半（`conic-gradient` + `transform: rotate()`）**不做**：
>    活跃角色的光环**存在**（复用既有的静态 `.neon-halo`），只是不动。
>    这一节真正关心的是「animate 渐变会让元素每帧重绘」，而我们干脆不动画它 —— 是更省的那一边。
>    打字光标仍然是纯 CSS 动画（技巧 2 完整落地）。
...
**M7 的决定（承接 §5.3，2026-09-24 从「建议」升级为「决定」）**：

> **M7 不设 `CLAUDE_CODE_MAX_CONTEXT_TOKENS`，也不传 `--autocompact`。**

① 那个变量是**阻断**阈值而不是压缩触发阈值，设小了得到的不是压缩而是**整轮失败**；
② `--autocompact` 的下限是 100k，小预算下**根本传不进去**；
③ 「真实窗口是多少」仍是 §2.4-3 的待办（`modelUsage.contextWindow` 自报 200000，
但那很可能是模型目录未命中时的兜底值）——**在一个未知的数字上做压缩协商，比不做更危险**。
⇒ M7c 的压缩**只按我们自己的条数阈值与字符估算触发**（§六 M7c），
并在归档里记一次「N 条 ≈ 多少 token」的标定映射，等窗口有实测值之后再谈协商。

> ✅ **M7c 已按这条决定落地**（2026-09-24）：`spawn` 那一行**没有加任何压缩相关的参数**，
> 压缩完全由 `compactAtCount` / `compactAtChars` 驱动（§4.6-1）。
> 走查的实测印证了决定的第 ① 条：真跑三轮里 **CLI 自己的 `compact_boundary` / `compact_result`
> 一次都没出现**（我们永远先压）—— 于是第 3 条修正里的「监听 `compact_boundary`，
> 出现即当 bug」在本轮**只有反面证据**（§六 M7c 实证）。详见 §8.9-13。
...
> ✅ **第 2 款在 M6b 落地**（顶栏常驻，概览屏与对话屏都在）。落地时有四条口径是**硬要求**，
> 每一条都是为了让那个数字**不撒谎**：
>
> - **标签写「估算」，不写「花费」。** 这是 §2.4-2 的直接后果：走第三方端点时
>   `result.total_cost_usd` 的数值不可信（它是按 Anthropic 官方价目算的，不是实际账单）。
>   `title` 里要把这句话写全 —— 写「累计成本」而不写口径，等于让用户以为那是账单。
> - **累计值只能来自聚合查询**，不能拿 `turn:list` 求和：后者**带 `limit`**，历史一长
>   那个和就悄悄变小，而它看起来完全正常（§4.6a 规则二）。
> - **`SUM` 跳过 `NULL`，所以聚合必须同时返回 `turnsWithoutUsage`**：
>   失败的、被中断的、被杀掉的那些轮次 `cost_usd` 就是 `NULL`，跳过它们得到的是一个
>   **偏低**的数字。非零时界面如实附一句「另有 N 轮无用量数据」（同上）。
> - **刷新时机只有两处**：每轮终态（`stream:status`）后一次、切空间时一次。
>   不加定时器 —— 成本不是每秒都在变的东西，而一个定时器会让「常驻」变成「常刷」。

> ⚠️ **第 4 款在本端点上判不了 —— 这一条由 M7a 探针定案，M7c 又确认了一遍**（2026-09-23 / 09-24）。
> 先说**原始事实**：M7a 的走查（2 轮）与 M7c 的走查（3 轮）加起来**五次请求，
> `cacheRead` 与 `cacheCreation` 全部是 0**（读数见 §六 M7c 实证的标定表）。
> 但**归因**不是「我们的前缀没命中」，而是「这个端点上这个字段不是那个意思」——
> 四条同日证据（逐字见 §8.9-16 与 `scripts/m7a-walkthrough.ts` 的判据注释）：
>
> 1. **自定义前缀 + 新进程 = 0**（两次独立运行，系统提示词**逐字节相同**、工具表相同 ⇒
>    本该命中的那段连续前缀一模一样，仍然 0）。M7c 的三轮是第 3、4、5 次确认。
> 2. **只有同进程续写才非零**（同一份归档里，第二条 stdin 行的 `cache_read=21120`）。
> 3. 而那个 21120 **大于**该进程第一次请求的 `input_tokens`（20181）—— 算术上不可能是
>    「本请求前缀里命中的一段」。
> 4. pre-M7a 那个 `cacheRead=37248` 是**跨项目、跨提示词、跨进程的常量**（m6a/m6b 四次运行全是它），
>    且只在**系统提示词是空串**时出现。
>
> ⇒ 「`cacheRead` 非零」在本端点上**不可作为验收判据**（§七-10 因此在本端点判不了），
> 而不是「M7a/M7c 把缓存弄坏了」—— 装配侧的前缀纪律仍然在（`envBlock` 不含时间戳、
> 历史纯追加、系统提示词两轮等长），**只是它在这个端点换不到证据**。
> **绝不许拿 M6b 那个 37248 当基线**：那是**另一个配置**（空系统提示词）下的数，
> 拿它对照会把一条端点事实判成我们的 bug。
...
> ★ **M7a 收口（一句话，但它是个真的后果）**：库里的 `role='system'` 行**不是**作为
> CLI 的 system 消息发出去的 —— 探针实测内层 `role` 只能是 `'user'`，写 `system`
> 会被**直接拒**（§六「M7a 实证」）。所以这一节的「持久化为 `message(role='system')` 行」
> 落地形态是：**它进的是历史数组**，由 `historyLabelOf()` 打上 `【系统】` 标签、
> 作为那**一条**扁平 user 行里的一段正文发出去。语义没变（它仍然是一个「该让模型知道」的事件），
> 但它**没有**任何系统提示词层面的特权 —— 与别的话一样躺在对话里。
...
| **M6b** | ✅ **已完成** `live` slice + 流式渲染 + 历史重放 + 成本常驻显示；`stream:status` 的**生产者**（M6a 剩的那个死通道）一并补上。 | **458 个用例全过**（M6a 的 365 个仍全绿 + 93 个新增），两个 tsconfig 项目 typecheck 干净。**实机走查全过**（`npm run walk:m6b`，真花钱，归档 `scripts/evidence/m6b-*/`），中途抓到一个**真的内容丢失缺陷**并修掉。**逐条见下方「M6b 实证」。** |
| **M7a** | ✅ **已完成**（2026-09-23；与 M7b **合成一个 commit** `e5fb051` —— 两块咬在一起，拆不干净就如实合成）`context-builder` + 消息数组化 + `systemPrompt` + **两件未实测事实的探针**。 | 第 2 轮能正确引用第 1 轮；cwd 的 `CLAUDE.md` **只进一遍**（两个路径都剔，见 §8.5c-1）；★ `cache_read_input_tokens` **在本机这个第三方端点上不能用来验收前缀缓存**（自定义前缀在新进程里恒为 0，只有**同进程续写**才非零 —— 四条证据见 §5.4 与 §8.9-16，**别拿 M6b 的 37248 当基线**）。**逐条读数见下方「M7a 实证」。** |
| **M7b** | ✅ **已完成**（2026-09-24；同 `e5fb051`）`mention-service.ts` + `@` 派发 + 跳数记账 + **§4.5b 的三条熔断**。 | 见下方「M7b 实证」：① 多 `@` 广播 ② agent→agent 派发（标记块）③ 2 跳空转 → 可见警告 + 落库 system 事件 ④ 4 跳 → 链被终止、扇出被取消 ⑤ 去重与 `cc` ⑥ 并发 ≤ 3 |
| **M7c** | ✅ **已完成**（2026-09-24；commit `1d54173` —— 它**一并收口了 M7a / M7b / M7c 三块的文档**，M7a 的收口是拍板并入本轮的）**压缩**：`compaction-service.ts`（纯判决 + 确定性摘要）+ `process/compaction.ts`（执行）+ 阈值 + 落库 + `inject_mode` 的消费。**M7 的最后一块。** | 见下方「M7c 实证」：三轮真机，阈值注入 `N=2` ⇒ 第 1 轮 `nothing-to-fold`、第 2 轮水位线推到 2、**下一轮装配里 `<summary>` 在位（`summaryChars=149`）**、被压的两条不在数组里、**触发消息仍是 `full`**；第二次压缩后摘要里**仍有第一段**；记下了「N 条 ≈ 多少 token」的**标定映射**；★ `compact_boundary` / `compact_result` **一次都没出现**（预判如此，且**不许**为它调参数） |
...
### M6b 实证：实机走查（`npm run walk:m6b`，2026-09-23）
...
```
npm run walk:m6b          # npm run build + 真跑两轮（★真花钱：2 轮真 CLI，每轮 --max-budget-usd 0.50）
npm run walk:m6b:dry      # 同上，但只开界面、装配走 IPC，一轮 CLI 都不发 —— 零成本
npm run walk:m6b:replay -- --archive=scripts/evidence/m6b-<时间戳>   # 从归档重出报告，零成本
```

**六个归档，全在仓库里**（报告是归档的纯函数，§8.8e 规则六）：

| 归档 | 是什么 |
|---|---|
| `m6b-2026-09-23T15-36-00-584Z/` | **dry，2 条不通过**：选不中空间、点不开对话视图。**零成本拦住**，脚本自己印「先修地基，再花真钱」并停下 |
| `m6b-2026-09-23T15-37-19-839Z/` | **dry，1 条不通过**：受控输入探针回 `undefined`（页面侧读数取错了）—— 又是零成本拦住的 |
| `m6b-2026-09-23T15-37-47-776Z/` | **第一次真跑：抓到真缺陷**。场景 1 印出「**DOM 45 字 / 帧 66 字**」，缺的正是模型说的第一句（见下） |
| `m6b-2026-09-23T15-50-30-686Z/` | **第二次真跑**：缺陷已修（场景 1 是 45/45），但场景 3 的采集**量错了时刻**，那两条 ❌ 是假的 |
| `m6b-2026-09-23T15-54-55-146Z/` | **干净 dry**（16/16 个页面内辅助函数全跑通）—— 花真钱前的最后一道 |
| `m6b-2026-09-23T15-55-35-224Z/` | **第三次真跑：全部通过**（0 条无法判定）—— 这一份是 M6b 的证据 |

> ★ 与 M6a 一样，**把「零成本的失败」留在仓库里**：两次 dry 各拦住一个缺陷，
> 而它们一共花了 0 元。这正是 `--dry` 存在理由的实证 —— 一次 dry 的时间成本是分钟级，
> 一次「花完钱才发现采集器坏了」的成本是钱 + 一轮真实轮次的不可复现性。

**关键实测数字**（场景编号照 §七）：

| 项 | 实测 |
|---|---|
| 场景 1 · 批次 | `stream:batch` **12 批**；帧种类 `thinking×2 / thinking_end×3 / tool_start×2 / tool_result×2 / file_diff×1 / text×3 / usage×1 / done×1` |
| 场景 1 · **正文逐字** | **DOM 32 字 / 帧 32 字**（取的是「光标刚撤、卡片还在」那一刻）—— 正文逐字送达，且光标语义（说完了就停）也对 |
| 场景 1 · 光标 | `.typing-cursor` **见过**，观察结束于 `cursor-gone` —— 「还在流」这件事在界面上真的看得见 |
| 场景 1 · 思考 | 点开后 **23 字**（`Let me read a.ts first.`）；面板是 `thinking_end` 后自动收起的，拿到正文只能靠点开 |
| 场景 1 · 工具顺序 | Read@32 早于 Edit@94（按发生序）；diff 里就是那次 Edit，**没有行号** |
| 场景 1 · **30Hz 隔离** | 在途卡片被改写 **17 次**；**已落定卡片 0 次**、**消息列表本身 0 次**（用户气泡那一桶也是 0） |
| 场景 1 · 同源 | 帧 **32 字** / 库 **32 字**；终态行在 `done` 帧之后 **252ms** 落库 |
| 场景 3 · 抑制 | 切走期间 Nova 的 `workspace:unread` **1 次**、漏过来 **0 批** |
| 场景 3 · **切走期间的产出（问库问出来的）** | 库里多了 **18 条**事件行（28 → 46），其中正文 **8 条**；而这段窗口里我们收到的批次是 **0** |
| 场景 3 · ★★ **重放补上了那一段** | 切回之后抢到那一瞬：界面正文 **173 字**，而我们收到过的推送一共 **9 字** —— **多出 164 字**，它只可能来自 `stream:resume` 的返回 |
| 场景 3 · 实时恢复 | 切回之后又收到该轮次 **20 批**；轮次 2 终态 `done/complete`（切走没影响它跑完） |
| 场景 4 · 历史接管 | 重开后 **live=0 / 已落定 2 张** —— 重放与历史的重叠在界面上不成立 |
| 场景 4 · 历史完整 | 思考面板初值全收起（2 条有按钮）、点得开；Read / Edit / diff 都在；正文含**完整的那一段流式正文**（逐字）；`thinking_end` 未落库；`running`/`queued` 幽灵行 **0** 条 |
| 场景 5 · 成本 | 概览屏上就有：估算 **$0.0000**（跑之前）→ **$0.3279**（重开之后，它不随进程消失） |
| 场景 5 · 聚合 | `{turnCount:2, costUsd:0.327943, tokensIn:38914, tokensOut:1477, turnsWithoutUsage:0}` —— 与库里 `cost_usd IS NULL` 的轮次数**一致**（都是 0） |

**这一轮最重要的是：走查抓到了一个真的内容丢失缺陷，而它是唯一能抓到它的地方。**

第一次真跑印出「DOM 45 字 / 帧 66 字」—— 界面上的回答**少了一截**，而且少得看不出来：
它是一个**完整的、只是更短的**回答。根因在渲染层：`learnEpoch` 对「第一次学到纪元」
也报 `epochChanged: true`，而 `applyBatch` 把那句话读成了「清空缓冲**并且丢掉这一批帧**」——
进程启动后的第一批帧恰好永远走那一支，于是**每次开应用，那一轮回答的开头都会缺一段**。

修法是把三个判断**分开说**（`watermark.ts` 的 `planBatch`：作废**旧缓冲** ≠ 丢掉**这一批**），
并把它们从 `store/live.ts`（引不到裸 Node 里）搬进 `src/shared/` 让测试钉住。
两次新增用例是「★★ 纪元刚学到时：清的是**旧缓冲**，这一批的帧照样收」与
「★ 「没见到这一轮的第一帧」→ 不许**新建**缓冲」。

> ★ 这个缺陷的形状值得记下来：**它不报错、不留痕、界面看起来完全正常**，
> 只是内容悄悄少一截。三条 `seq` 轴、水位线、纪元这一整套机制都可能出这一类错，
> 而**单元测试覆盖不到渲染层**（`src/renderer/src/ipc.ts` 在模块作用域就 `createClient(window.api)`）。
> 所以它是在**走查里**被抓到的 —— 这不是运气：那段正文是**逐字比对**的（DOM vs 帧拼出来的），
> 一个「少 21 个字」的差在 45/66 两个数字上藏不住。

**同样是这一轮，走查脚本自己有三个缺陷，全部被修掉，且都没有冤枉产品**（§8.8e 规则三）：

1. **场景 3 的第一版判据是废的。** 它去量**轮次结束之后**的 DOM —— 那一刻界面显示的是**历史行**，
   而历史行的正文来自 `content_text`，**它本来就是完整的**，重放生不生效都长那样。
   改成「界面上的正文 > 我们收到过的全部推送」这个**单 tick** 的不等式（页面上一个函数里同时取两个数）：
   推送只会让正文变长、**永远不会让它超出自己**，所以多出来的那一段只可能来自重放。
2. **「切走多久」的收手条件拧反了。** 第一版把「还在产出事件行」当成继续待着的理由，
   于是一个从头到尾都在产出的轮次把窗口拉到上限 —— 而「一直在产出」恰恰是**最该回来**的时候。
   实测撞上：回来时那一轮已经 `status done`，缓冲被终态推送丢掉，一条读数都没抢到。
   改成「终态先到就早回；满最短时间且**切走期间真有正文落库**也回；上限兜底」。
3. **两条判据量错了时刻**：`sawCursor` 数的是「带正文的批次有几批」（该数的是**时间**：
   这个端点上正文与 `done` 只隔 1ms，两批也等于量不出来）；「思考面板点得开」读的是**点击当拍**的 DOM
   （React 在点击之后才提交，必然读到点击前的样子）。两条都改成**从归档里算**，而不是信当拍的读数。

> ⚠️ **走查之后、进库之前还改了一处，而它不在上面任何一次运行的覆盖里。**
> `ui.select()` 现在会顺手切回 `overview`：原先在对话视图里点侧栏的成员（或项目）
> 是个**静默的空点** —— `selection` 确实变了，而 `App` 按 `view` 渲染，那一屏还是对话，
> 点击看起来什么都没发生。修法与 `setView()` 清 `selection` 对称，两句合起来是
> 「**在看某一屏**与**在看某个对象**互斥」（§4.7）。
> 五个场景没有一个会从对话视图点侧栏成员，所以归档里的任何一条读数都没有覆盖它 ——
> **这一条是读代码时发现的，不是走查抓到的**。写在这里的理由是 §8.8e 规则三的同一个立场：
> 覆盖不到的地方要说出来，不能让「全部通过」被读成「什么都验过了」。

**M6b 的已知不做**（与 §4.8、§九一致，写在这里免得被当成遗漏）：不装 Shiki、不做光环旋转、
不做 Markdown 渲染（流式期间渲染原始文本）、不做停止/插话（M9）、不做 `@` 派发（M7）、
不拼历史上下文（M7）、blob 外置仍然没有（M6a 偏差一的延续：`blob_path` 恒为 `NULL`）。

### M7a 实证：探针 + 实机走查（`npm run probe:m7a` / `npm run walk:m7a`，2026-09-23）

M7a 有两半，**判据不同，分开说**：探针回答「CLI 的 stdin 到底收什么」（一次真跑的归档
`m7a-2026-09-23T16-51-08-216Z`，四臂 A/B/C/D），走查回答「我们的装配端到端对不对」
（`m7a-2026-09-23T17-09-53-546Z`，**全部通过，另有 1 条「无法判定」**）。

**① 探针：`--input-format stream-json` 的 stdin 收多条消息吗**

★ 这个问题的**失败模式不是「被拒绝」，是「被接受成 N 轮」** —— 后者不报错、看起来完全正常，
而 `claude-adapter` 在第一条 `result` 上就 `closeStdinOnResult`（默认 `true`），
于是 result #2..N 被我们自己掐掉，症状是「一轮用户消息变成一条被折叠的助手消息，成本只记了 1/N」。
所以**单条价值最高的断言是：数归档里 `type:"result"` 的行数**。

| 臂 | 写进 stdin 的 | `result` 条数 | 判决 |
|---|---|---|---|
| **A** | 3 行，中间那行是 `role:'assistant'` | **0** | **拒**：`Error: Expected message role 'user', got 'assistant'` |
| **B** | 1 行：`renderTurnInput()` 的现产扁平文本 | **1** | 生产形状 —— **唯一能承载一轮上下文的形状** |
| **C** | 2 行，第 1 行是 `role:'system'` | **0** | **拒**：同形报错（§5.6 的落法因此要改） |
| **D** | 3 行，**全是 `user`** | **2** | ★★ 切在 `1 \| {2,3}` —— **这一条把原来的断言推翻了** |

★★ **D 臂是本里程碑最重要的一条否证**（A 同时改了两件事 —— 行数与 role —— 而报错只指向 role，
于是「3 行 user 会怎样」在 A 那里是空白；D 与 A 只差那一个 role 字节，变量被隔离出来）：

| D 的 result | `input_tokens` | `cache_read_input_tokens` | 读法 |
|---|---|---|---|
| #1 | 20181 | 0 | 第 1 轮的模型**只看得见第 1 行**那个标记 |
| #2 | 301 | **21120** | 第 2 轮看得见**全部三条**，且**接着第 1 轮的会话在跑** |

⇒ 这个协议是**多轮对话**协议，不是「一轮的多条消息」协议。
⇒ 3 行给 2 条（**不是 3 条**）说明**切点由缓冲区里已经躺了几行决定，不归我们管** ——
「取决于时序」比「一定是 N」更糟：换台机器、换个负载就可能给出不同的轮数。
⇒ **对 M7a 的后果**：`context-builder` 的产物必须是**一条** user 行，由 `renderTurnInput()` 那样拼成
（纯追加的 `join('\n\n')`，第 N 轮是第 N+1 轮的字节前缀）。§4.4e 的「`run()` 恰好一次 `done`」
**没有被推翻**，但**保住它的原因变了**：不是 CLI 保证一行的上下文只算一轮，而是**我们只发一行**。

**② 探针顺带补测：还有哪些项目记忆会被 CLI 自动注入**（判据是**行类型**：标记首次出现在
`assistant` 行且**之前没有 `tool_result`** ⇒ 一开始就在上下文里，不是模型自己读来的）

| 文件 | 归档里的读数 | 结论 |
|---|---|---|
| cwd 的 `CLAUDE.md` | `MK-ROOT-` 在 `assistant` 行，之前无 `tool_result` | **注入**（**阳性对照** —— 对照不成立时，下面两条「没被注入」全都不可信） |
| `.claude/CLAUDE.md` | `MK-DOTCLAUDE-` 同上 | ★★ **也注入** —— §4.2 逐字警告的「只剔根那一份就仍是两遍」**说中了** |
| `AGENTS.md` | **两条臂的归档里一次都没出现** | **不注入** |
| `--add-dir` 目录里的 `CLAUDE.md` | （§8.5c-1 的 M5 实测）整轮从未进上下文 | **不注入** |

**③ 走查：端到端那一半**

台子：一个空间（Nova）、一个成员（Atlas）、两轮真 CLI。行为判据**全部锚在只可能来自历史的串上** ——
第一轮的用户消息里埋 `MK7A-R1-<后缀>`，第一轮的回答里要模型**自己编**一个 4 位数（本次 `7314`）。

| 断言 | 归档里的读数 |
|---|---|
| 第 1 轮装配形状 | 人设 **131 字节**进了上下文、`systemPromptChars=267`（M6a 那一版是**空串**，这是回归判据）、`roleDescBytes === null` ⇒ `<role>` **整块不出现** |
| ★ 第 1 轮**只用一轮的历史** | `historyIncluded=1 / historyTotal=1 / messageCount=2` |
| ★★ **第 2 轮记得第 1 轮** | `7314` **两轮的提示词里都没有**（两轮都查过）⇒ 唯一来源是**第一轮的回答作为助手消息进了数组**；第 2 轮**答出了它**。这是本里程碑的头号验收 |
| 用户历史行也在数组里 | 第 2 轮也答出了 `MK7A-R1-…`（那是第一轮的**用户消息**） |
| ★ 追加式、稳定前缀 | `messageCount` 2 → 4、`contentChars` 4946 → 5207、而 `systemPromptChars` **267 → 267**（★ 等长是**必要条件不是同一份** —— 「逐字节相同」要 hash，`[ctx]` 里没有） |
| cwd 的 `CLAUDE.md` 只进一遍（**我们这一半**） | `suppressedAutoInjected=2`、`projectFiles=2`（`AGENTS.md` + `.claude/rules/notes.md`）、项目记忆 **11295 字节** |
| cwd 的 `CLAUDE.md` 只进一遍（**CLI 那一半**） | 提示词里**不含** `MK7A-CWD-…`，而模型**逐字复述了它** ⇒ 来源只能是 CLI 自己带的（**剔除是对的**） |
| `.claude/CLAUDE.md` | 同样被复述 ⇒ 与探针的补测互证 |
| ★ 阳性对照 | 我们注入的 `.claude/rules/notes.md` 标记被复述 ⇒ `<project_context>` 这条注入缝**真的通了**，上面几条「没被注入」的结论才有依据 |
| 前提 | 第 1 轮 `tool_start = 0` —— 模型一旦自己 `Read` 过 `CLAUDE.md`，复述就毫无信息量；**前提不成立时这两条判降级成 `unknown`，不照印 ✅**（§8.8e 规则三） |
| 库里的事实 | `turn` 2 条、`message` 4 条（user 2 / assistant 2，`seq` 1→2→3→4）、`hop_depth` 全 0；★ 第一轮落库的助手正文与流出来的**逐字相同**（142 / 142 字符） |
| 生产路径上的 warn | `context:cwd-claude-md-suppressed` + 2× `adapter-diag:result-subtype`；**没有** `history-window-capped`、**没有** `history-missing-trigger` |

**④ 缓存那一格（**端点相关**，§5.4 / §8.9-16）**

走查两轮 `cacheRead` 都是 **0**，而同一天的探针 D 臂在同进程续写时读到 **21120**。
两件事**不矛盾**：这个字段在本端点**不能用来验收前缀缓存** ——
自定义前缀在**新进程**里恒为 0、只有**同进程续写**才非零、且那个非零值（21120）**比它自己第一次的
`input_tokens`（20181）还大**、而 M6b 的那个 37248 只出现在**空系统提示词**的配置里。
⇒ 如实记为「**判不了**」，**不是「测了没事」**；★ **绝不许拿 37248 当基线**。

**M7a 的已知留白**（写在这里免得被当成遗漏）：
`cacheRead` 那一条（原始事实是 0，只是归因不了）；行类型判据的那一半在**探针**里、
走查做的是端到端那一半；`AGENTS.md` 在走查里**分不清是谁注入的**（我们注入了它，CLI 也可能带）。
`compile` 层的 `history-window-capped` / `history-missing-trigger` 两条 warn 在本次规模下**没被触发**
（三条历史远不到取数上限）—— **没被触发不等于对**，它们由单测守着。

### M7b 实证：实机走查（`npm run walk:m7b`，2026-09-24）

**一共跑过四次**（三次真跑 + 一次 dry），归档都在 `scripts/evidence/m7b-*/`。
**最终那一次**（`m7b-2026-09-23T18-01-02-681Z`）的结论是**全部通过（另有 2 条「无法判定」）**。
★ 前两次真跑报的 ❌ **全是走查脚本自己的错**（正文多拼了一个 `@Echo` ⇒ 去重不命中；
按会话扫计数 ⇒ 把别的链算进来），**`src/` 一个字节没动** —— 六条纪律记在 §8.8h。

**A 空间（Sable，4 个成员）：多 `@` 广播、去重、`cc`**

| 断言 | 归档里的读数 |
|---|---|
| 一条消息 `@` 三个成员 | `Atlas@0` + `Nyx@1` / `Echo@1` / `Juno@1`；**每条派发轮次的触发消息都是「那一跳的转述行」**（`role:'user'`、作者是 Atlas、`seq` 2/3/4），不是用户那条消息 |
| 转述正文 | 「【派发】Atlas 在**刚发的那条消息里** @ 了你，把这件事交给你。」+「—— 以下是原文 ——」+ **用户原文逐字**（含 `@Nyx @Echo @Juno` 那几个字） |
| 两次发送**正文逐字相同** | 归档里两条 `role:'user'` 行（作者都是 Atlas，`seq` 1 与 7）**正文完全相同**（48 字）；两条 `Atlas@0` 轮次 |
| 去重命中 | `fanout:mention-deduped`，指名 **Juno**、理由「还没开始跑」（它还在 `queued` 尾部）⇒ Juno 只有 **1** 条转述 / 1 条轮次 |
| ★ **反向对照（实测到的那一半）** | **Nyx 拿到 2 条转述 / 2 条轮次** —— 第二次发送时它第一轮**已不在** `queued` 尾部，于是**照样被派**（§4.5b 第 2 条：已经执行过 ⇒ 可以再派） |
| `cc` 不建轮次 | 第二次发送里 Echo 只多了一条「【抄送】Atlas…**你不需要行动**，只是让你知道。」的历史行，**轮次仍是 1 条** |
| 自我 `@` 被拦 | `fanout:mention-self`：Nyx 的回复里写了 `<mentions>Nyx</mentions>`（转述正文逐字带着 `@Nyx`，模型照着回 `@`）⇒ 跳过，**没有自己派给自己** |

**B 空间（Rill，2 个成员）：agent→agent 派发 + 三条熔断里的两条**

链的形状（`db-A-chain.json` 的 `turn.hop_depth`，按 `queued_at`）：

```
Vex@0 (e3cebd79) → Wisp@1 (2f68f70c) → Vex@2 (fd4eead8) → Wisp@3 (2b11632c)
```

| 断言 | 归档里的读数 |
|---|---|
| agent→agent 从标记块来 | `Vex` 的回复末尾是 `<mentions>Wisp</mentions>` ⇒ `Wisp@1` 被建；链上每一跳都这样接下去（**这是 M7b 最核心、也最贵的一条**） |
| 转述剥掉标记块 | 下游读到的是「【派发】Vex 在**它上一轮的回复里** @ 了你…」+「（它原文末尾的 `<mentions>` 那一行已由本应用取出 —— 那是派发指令，不是给你看的。）」+ **原文去掉那一块之后**的正文 |
| **2 跳空转 → 警告 + 落库** | `fanout:ping-pong-warn`：`emptyHops: 2`，`chain: [Vex@0 hadWork:false, Wisp@1 hadWork:false]`；**Wisp 的会话里多了一条 `role:'system'` 行**（`seq` 5），正文**不含任何跳数数字** |
| **4 跳空转 → 终止** | `fanout:ping-pong-terminate`：`emptyHops: 4`，链为那四跳；Wisp 的会话里第二条 system 行（`seq` 10）写着「已被自动终止 —— 本轮的派发已取消，还在排队的同链条轮次也已取消」 |
| ★ **终止真的执行了** | `Wisp@3` 的回复**又写了 `<mentions>Vex</mentions>`**，而**没有任何 `Vex@4` 被建** ⇒ 终止分支（**不调** `fanoutOf`）确实拦住了那一跳 |
| 「实质工作」在这一跑里恒为假 | 整个归档的 `tool_start` / `file_diff` 事件数是 **0** —— 这一跑刻意要求「不要使用任何工具」，于是每一跳都是空转，正好把熔断逼出来 |
| ⚠️ 一处**措辞重复**（记为待清理） | 终止那条 system 行把 `verdict.message` 又接了一遍，读起来是「…也被取消。**这条协作链连续几跳都没有任何实际动作（既没有工具调用，也没有文件改动），本轮之后不再往下派发。**」—— 语义不重、但冗。产品缺陷，不是采集问题；**没改**，记在这里 |

**两条 ⚠️（「无法判定」，不是失败）**

1. **「终止时取消排队中的同链条轮次」在线性链下不可达** —— 链上每次只有一跳在跑，
   它后面没有兄弟排队跳。这一条由 `test/process/fanout.test.ts` 覆盖，**不靠真机**（§4.5b-1 末）。
2. **`fanout:mention-self` 是模型侧行为** —— 它在这一跑里**出现过一次**（Sable 的 1 跳那一轮），
   产品拦下了它；但走查**判不了它会不会出现**，它不是验收项。

**并发 ≤ 3**：A 空间那一次是三条派发轮次同时排在队里，观察到了排队（Juno 被去重时
「还没开始跑」就是排队在档里的痕迹），槽位一次都没有被突破。

**M7b 的已知不做 / 已知有损**（与 §4.5b-1、§九一致，写在这里免得被当成遗漏）：
不加全局 hop 上限；不判「有新信息」（只判「有动作」，**方向偏严**）；
去重基准是**进程内**的（重启后为空）；`cc` 不建轮次；
运行中的轮次**不 abort**（那是 M9 的活）。

### M7c 实证：实机走查（`npm run walk:m7c`，2026-09-24）

**跑过三次**：一次干跑（**失败**，原因在走查脚本自己身上，见下）、一次干跑（全绿）、一次真跑。
归档都在 `scripts/evidence/m7c-*/`，**最终那次**（`m7c-2026-09-24T16-20-01-501Z`）**全部通过**。

台子：一个空间（Vela）、**一个成员**（Atlas，人设刻意不提 `@`）、三轮真 CLI。
阈值由 `CODE_CHAT_COMPACTION_N=2` 注入（默认 20 —— 三轮根本够不着，所以「压了」本身就证明旋钮被采纳）。

**① 判决与落库（库里看得见的那一半）**

| 断言 | 归档里的读数 |
|---|---|
| 第 1 轮**不该压** | `compaction:decided → nothing-to-fold`；`compacted_through_seq = 0`、`rolling_summary = NULL`；会话里 2 条消息**全是 `full`** |
| 第 2 轮**跨过阈值** | `compaction:decided → count-threshold` ⇒ `compaction:compacted`「已折叠 2 条骨架，水位线推到 2」；`rolling_summary` 126 字 |
| ★★ **触发消息不进摘要** | 第 2 轮的触发消息 `seq=3`、`inject_mode` **仍是 `full`**，水位线只到 **2** —— 区间右端是**开**的（`lastSeqBefore` 是它唯一的所有者） |
| 被折的**恰好两条** | `seq=1,2`（第 1 轮的请求与回复）被标 `summary` |
| 可见的那一行 | 一条 `role:'system'` 行，**`turn_id IS NULL`**（带上就会被 `listByTurn` 算成那一轮的产物） |
| ★★ **累积** | 第 3 轮收尾又压一次（水位线 **5**，折了 3 条）⇒ 最终摘要里**仍有第一段**、也有第二段，且**只有一条标题行** |
| 只改状态不删数据 | 会话里 8 条消息（`summary` 5 / `full` 3 / `excluded` 0），`deleted_at` 全为 `NULL` |

**② 「压缩真的生效」—— 只在下一次装配里看得见的那一半**（`[ctx]` 落在 `app-A.stdout.log`）

| | `compactedThroughSeq` | `summaryChars` | `historyIncluded/Total` | `contentChars` |
|---|---|---|---|---|
| 第 1 轮 | 0 | **0** | 1 / 1 | 370 |
| 第 3 轮 | **2** | **149** | 4 / 6 | 618 |

第 3 轮的 `historySuppressed = 2` ⇒ 被水位线挡掉的两条**真的不在数组里**，
而不只是「库里标了个状态」。★ 这一格是**唯一**能证明压缩生效的地方 ——
库里写了而装配没读，它会是 0；只取库里那一半，这一半的失效**不会被任何别的判据发现**。

**③ 标定映射（shape ↔ usage）—— ★ 这是**上界**，不是「我们拼了多少 token」**

| 轮次 | `usage.in` | `cacheRead` | `cacheCreation` |
|---|---|---|---|
| T1 | 18709 | 0 | 0 |
| T2 | 18746 | 0 | 0 |
| T3 | 18869 | 0 | 0 |

★ 口径：`usage` 是 CLI **整条请求**的用量，**包含 CLI 自己加的 system prompt 与工具定义**
（约 18.7k token，而我们的正文只有 370–618 字）—— 所以这是**上界**，两者不能互相换算（§5.3a）。
★ 三次 `cacheRead` 全是 0：这是 §5.4 那四条证据的第 3、4、5 次确认，**不是**「M7c 弄坏了缓存」。

**④ 预判兑现：CLI 自己的压缩一次都没出现**

`compact_boundary` / `compact_result` **零次**，`app:notice` **零条**。
理由写在走查文件头里，是**算术的**：我们阈值 2 条、CLI 的以万级 token 计，我们永远先压。
⚠️ 所以这一条是「**没观测到**」，不是「不存在」，**它只在这个沙箱历史规模下成立**；
而且**不许**为了让它露面去调参数 —— 那会把一条诚实的结论换成一条造出来的（§8.9-13）。

**⑤ 干跑（零成本）真的取证了两件事**

1. **静态接线**：`runtime.ts` 里 `createCompaction` 在、且 `compaction.onTurnFinished(turn)`
   **排在 `fanout.onTurnFinished(turn, info)` 之前**，`main/index.ts` 里读 env 旋钮、透传阈值。
   ★ 这是**唯一**能判「顺序对不对」的手段 —— 那两行互换**不会红任何测试**（§4.5a 规则一
   保证同会话不会并起第二轮），所以只能读源码。
2. **旋钮真的被应用读到了**：干跑刻意用一个**非法值** `abc`，主进程 stderr 里因此出现
   `[compaction] CODE_CHAT_COMPACTION_N="abc" 不是一个正整数，这一跑用默认阈值 20`。
   ★ 这是零成本下唯一能证明「这个变量被读进去了」的观测：**有效值不产生任何输出**，
   于是「设了但没人读」与「设了且被采纳」在没有这一枪时**长得一模一样**。

**⑥ 这一跑抓到的两件事（都如实记，都没改）**

1. **我们自己的折叠说明会被再折一次**（长这样：`[5] 【系统】 【系统】历史上下文已压缩：seq ≤ 2 …`），
   为什么明知而不改，见 §8.9-21。
2. **走查脚本的一处竞态**（不是产品缺陷）：CDP 调试目标一出现就注入页面采集器，
   有时会打在一个**尚未提交**的文档上 —— 那一刻 `page.url` 已经是 `index.html`，
   写入却随加载被吃掉，两三句之后才在别处炸成 `undefined`。
   第一次干跑就是这么挂的（归档 `m7c-2026-09-24T16-18-30-774Z`，13 条 ❌ 全是它的后果）。
   修法与纪律见 §8.8i。
...
**M6b 验证**（这一层**第一次有界面可见的变化**，所以断言从「库里的行」变成了「界面上的字」）：

```bash
npm run walk:m6b:dry      # ★ 先跑这个：只开界面、装配走 IPC，一轮 CLI 都不发（零成本）
npm run walk:m6b          # 真跑：2 轮真 CLI（第 2 轮是八文件长读，够长到能切走再切回），每轮 --max-budget-usd 0.50
npm run walk:m6b:replay -- --archive=scripts/evidence/m6b-<时间戳>   # 重放：零成本重出报告
```

五个场景照计划 §七：① 正文逐字 / 光标 / 思考 / 工具 / diff；② 30Hz 隔离（在途卡片改、其余零改）；
③ 切走抑制 + 切回重放；④ 硬杀重开的历史完整重放；⑤ 成本常驻显示。
**驱动一律走界面**（点击 / 输入 / 切空间），装配走 IPC —— 夹具不是被测物。

★ **这一层唯一能测的地方就是走查**：`live` slice 引不到裸 Node 里
（`src/renderer/src/ipc.ts` 在模块作用域 `createClient(window.api)`），
所以凡是「纯函数」的部分才搬进 `src/shared/live/` 让 `npm test` 钉住，
剩下的判据必须建立在**界面上的字**与**归档里的事实**之间。M6b 那个真缺陷正是这样被抓到的
（见 §六「M6b 实证」）。

同样**绝不进 `npm test`**；证据归档在 `scripts/evidence/m6b-<时间戳>/`，**报告是它的纯函数**。
⚠️ 与 M6a 同一条：归档里的 `console.log` / `app-*.log` 被根 `.gitignore` 的 `*.log` 吃掉，
而报告一个字节都不读它们（详见下面 M6a 那段里的说明）。

**M7 验证**（三块各自一个走查脚本，块与块的顺序是硬的 `a → b → c`）：

```bash
npm run probe:m7a:dry && npm run probe:m7a   # M7a 的探针：★唯一花钱的前置，3 臂各一道预算
npm run walk:m7a:dry  && npm run walk:m7a    # 历史数组化 + systemPrompt + cwd 的 CLAUDE.md 只进一遍
npm run walk:m7b:dry  && npm run walk:m7b    # @ 派发 + 三条熔断（★三块里最贵：一轮 @ 出去就是好几跳）
npm run walk:m7b:replay -- --archive=scripts/evidence/m7b-<时间戳>   # 重放：零成本重出报告
```

`walk:m7b` 的六个场景照计划 §七：① 多 `@` 广播 → 多条轮次；② **A 的回复里 `@` B → B 被派发**
（最核心最贵的一条）；③ 2 跳无实质工作 → 界面警告 + 库里一条 `role:'system'`；
④ 4 跳 → 链被终止、扇出被取消；⑤ 同一 `(成员, 内容)` 在 `queued` 尾部 → 只建一条；
⑥ 广播时同时 `running` ≤ 3。**逐条读数见 §六「M7b 实证」。**
★ `--dry` 在这里比别的里程碑更值钱（最贵），但它挡的是**采集通道**的坏；
**采集判据**的坏要等真跑才现形 —— M7b 的三条 ❌ 全是这样现形的（§8.8h）。

**M7a 验证**（这一层的产物**在界面上完全看不见** —— 装配是「发出去的东西」，不是「流出来的东西」）：

```bash
npm run probe:m7a:dry && npm run probe:m7a          # 探针：四臂，★每条臂各一道预算（真花钱）
npm run walk:m7a:dry  && npm run walk:m7a           # 走查：2 轮真 CLI
npm run walk:m7a:replay -- --archive=scripts/evidence/m7a-<时间戳>   # 重放：零成本重出报告
```

探针判的是「**CLI 的 stdin 收什么**」（四臂的 `result` 条数与 `cache_read_input_tokens`），
走查判的是「**我们的装配端到端对不对**」（第 2 轮记得第 1 轮、人设进了、cwd 的 `CLAUDE.md` 只进一遍）。
**逐条读数见 §六「M7a 实证」。**

★ 一条**会误导后来人**的端点事实必须记在这里：本机这个第三方端点的 `cacheRead`
**不能**用来验收前缀缓存 —— 自定义前缀在**新进程**里恒为 0、只有**同进程续写**才非零，
且那个非零值比它自己第一次的 `input_tokens` 还大，而 M6b 的 37248 只出现在**空系统提示词**的配置里。
证据在 `scripts/evidence/m7a-2026-09-23T16-48-08-427Z` 与 `…16-51-08-216Z`；见 §5.4 与 §8.9-16。
⇒ 这一层在 `cacheRead` 上**留白**（原始事实是 0，只是归因不了），**不许记成「测了没事」**。

**M7c 验证**（压缩：**有一半在界面上完全看不见** —— 库里写了什么、下一轮装配读了什么）：

```bash
npm run walk:m7c:dry      # ★ 先跑这个：静态接线 + 旋钮被采纳 + 水位线仍为 0（零成本）
npm run walk:m7c          # 真跑：3 轮真 CLI（第 2 轮跨过阈值，第 3 轮的装配里才看得见效果）
npm run walk:m7c:replay -- --archive=scripts/evidence/m7c-<时间戳>   # 重放：零成本重出报告
```

★★ **一条纪律（这一层唯一靠得住的验收形态）：压缩的正确性只能由两条_互相独立_的事实合判 ——
①下一轮 `onContextBuilt.shape` 里的 `summaryChars` / `compactedThroughSeq` / `historyIncluded`；
②库里那条会话行的 `compacted_through_seq` 与逐条 `inject_mode`。
绝不许拿「界面看起来对」当证据。** 理由不是教条，是这两条**各自都能单独失效而另一个毫无反应**：

- 库里写了水位线、装配层**没读** ⇒ 界面**完全正常**（历史照样全量注入，只是变长），
  除了模型自己变笨，没有任何信号；
- 装配读了、库里**没写** ⇒ 也不报错 —— 读侧只认水位线，水位线永远是那个「落后但合法」的旧值。

而**界面上能看见的只有那条折叠说明的 `role:'system'` 行**（以及 `app:notice` 的 warning），
它**只证明写侧跑过**，**一个字节都不证明**下一轮装配用上了它。

★ `--dry` 在这一层的证明力是**有限的、且要说清楚**：
它证明**静态接线**（`compaction.onTurnFinished(t)` 排在 `fanout.onTurnFinished(t, info)` **之前** ——
那两行互换**不会红任何测试**，所以只能读源码断言）与**旋钮真的被应用读到了**
（刻意喂一个非法值，主进程 stderr 里出现 `[compaction] … 不是一个正整数 …`；
★ **有效值不产生任何输出**，所以没有这一枪时「设了但没人读」与「设了且被采纳」长得一模一样）。
它**不能**证明压缩的判断对 —— 那只有真跑才露出（§14.6）。
...
10. **成本**：确认 UI 显示累计 token 与估算成本，且 `cache_read_input_tokens` 非零
    > ✅ **M6b 落地了显示这一半**：顶栏常驻「估算 $X.XXXX」，`title` 里带「入 N / 出 N token」
    > 与口径（按 Anthropic 官方价目算的估算值，不是实际账单）。口径与三条硬要求见 §5.4。
    > `cache_read_input_tokens` **实测非零**（M6b 两轮：`cacheRead=37248` / `155648`），
    > 所以在**本端点下**缓存读确实命中过；判据仍是「恒为 0 才算 bug」。
    > ⚠️ 累计值来自聚合查询，而 `SUM` 跳过 `NULL` —— 界面必须同时显示 `turnsWithoutUsage`（§4.6a）。
...
> ✅ **M7a 收口：这张表补全了，而且 §4.2 那句警告说中了**（2026-09-23，`scripts/m7a-probe.ts`）。
>
> §4.2 逐字警告过「**只剔根那一份就仍是两遍**」。探针把四个路径逐个测了一遍
> （判据仍是**行类型**：标记第一次出现在 `assistant` 行、且之前没有任何 `tool_result`）：
>
> | 文件 | 会不会被 CLI 自动注入 |
> |---|---|
> | `<cwd>/CLAUDE.md` | **会**（M5 已实测） |
> | `<cwd>/.claude/CLAUDE.md` | ★ **会**（M7a 探针补测）—— 警告说中的就是这一条 |
> | `<cwd>/AGENTS.md` | 不会 |
> | `--add-dir` 目录里的 `CLAUDE.md` | 不会（M5 已实测） |
>
> **所以「M7 还要不要注入 cwd 那一份」的答案是：那两个不注入、其余照常注入。**
> 依据与理由写在 `context-builder.ts` 的 `AUTO_INJECTED_FROM_CWD` 上，
> 一句话概括是**代价不对称**：多注入一份的代价是浪费 token（**可恢复**），
> 少注入一份的代价是模型以为项目没有记忆、于是自己造一份约定（**不可恢复**）。
> 剩下四个路径（`.claude/rules/*.md`、`.cursorrules`、`.cursor/rules/*`、
> `.github/copilot-instructions.md`）**没有实测**，因此按「多注入」那一侧兜底 ——
> 这是**明知的未知**，登记在 §14.3。
...
七条全部来自这一轮**真机跑出来的错**（三次真跑 + 两次 dry）：一、二、四、五、七条是
**走查脚本与采集判据**的错（它们如果把「我没采到」印成「产品坏了」，整轮结论就废了），
三、六条是**产品代码**的错 —— 其中第六条是 M6b 唯一一次真的丢内容。

**一、`tsc` 不检查模板字符串里的内容 —— 而它报出来的错会在几百行之外。**

走查脚本把页面侧 JS 装在一个大模板字符串里（`HELPERS`），然后用 `evaluate()` 注进去。
**`tsc` 对模板里的内容一无所知**，而一条页面侧注释里**一个裸反引号**就会提前把模板截断 ——
于是整个文件从那里往下错位，报出来的是 `TS1005`、`TS1443`，行号指在**几百行之外**的某个
看起来完全正常的地方。这一轮踩了**两次**（第二次是刚写完一条关于它的注释）。

> 规矩：**碰过 `HELPERS` 就必须跑一次 `npm run check:helpers`**
> （`scripts/check-page-helpers.cjs`，把那段模板抠出来单独做语法检查），
> 而且**不许在页面侧注释里写反引号** —— 写 `drain()`、`sync()`，不写 `` `drain()` ``。
> 这条注释本身也只能这么写，这不是风格洁癖，是这个形状的模板字符串的物理约束。
>
> ⚠️ 那个脚本**抠区间取不到就报错退出**，还会验一个标记（`window.__m6b`）：
> 悄悄抠错一段然后印「语法 OK」**比不检查更坏** —— 它会让人以为检查过了。

**二、「preload 的 `window.api` 落地」早于「React 挂载过了」—— 两者不是一件事。**

「桥回来了」≠「界面画好了」。更隐蔽的是夹具：走查的沙箱数据是渲染进程起来**之后**
通过 IPC 装配的，而**没有任何推送告诉渲染进程**。所以「装配完了」与「界面上看得见」
之间必须显式插一次 `reloadRenderer()`，否则所有 DOM 断言都在对着一个空界面说话。

**三、要比较的两个数必须在**同一个 tick**里取。**

「界面上的正文」与「我们收到过的推送」这两个数，分开两次 `evaluate()` 取就会
在中间夹进几十毫秒的新推送，那个差就什么都不证明了。页面侧 JS 是单线程的，
一个函数体里没有 `await` 就没有东西能插进来 —— 所以判据要写成**页面侧的一个函数**，
而不是采集侧的两次往返。

**四、点击之后立刻回读 DOM，必然读到点击前的样子。**

`.click()` 是**同步派发**，React 在**这一拍之后**才提交。所以「点开了没有」这个读数
不能在点击的那个函数体里取 —— 一次真机跑就是这么把一次**成功**的点开说成「点不开」的
（`stateAtClick: still-closed`，而 200ms 之后那一行开着、里面 23 个字）。
判据要读**睡过一觉之后**的快照，或者干脆从归档里两份快照的差算 —— 后者还能零成本重判。

**五、「量不出来」的判据要量「时间」，不是量「批数」。**

这一轮把「光标活不过一拍」写成了「带正文的批次 ≤ 1」，看着挺像，**但数错了**：
这个端点上正文恰好来了 2 批（29 字 + 16 字），看着像流了挺久，而两批相隔 0ms、
`done` 在 1ms 之后 —— 整段正文与 `done` 落在**同一瞬**。
于是它把「量不出来」判成了「光标没在工作」，凭空长出一条 ❌。
**能决定「量不量得到」的只有时间**：从第一个带正文的批次到 `done` 之间有多宽。

**六、★ 「作废旧缓冲」与「丢掉这一批帧」是两件事 —— 揉成一句的代价是静默的内容丢失。**

这是这一轮唯一的产品缺陷，形状值得完整记下来：

`learnEpoch` 对「**第一次**学到纪元」也报 `epochChanged: true`（那是真的 ——
它确实变了，从「没有」变成了某个值）。而 `applyBatch` 把这句话读成了
「清空缓冲**并且丢掉这一批帧**」。**进程启动后的第一批帧恰好永远走那一支**，
于是每次开应用，那一轮回答的**开头**都缺一段。

它的表现是「界面上的正文 45 字 / 帧 66 字」—— 一个**完整的、只是更短的**回答。
不报错、不留痕、看起来完全正常。

> 修法不是改一个 `if`，是**把三个判断分开说**（`watermark.ts` 的 `BatchPlan`：
> `resetBuffers` / `useFrames` / `createBuffer`），并且让 `useFrames` **恒为真** ——
> 想让它把帧也丢掉，就得先把这个字段改成假，而**测试会当场拦下**。
> 一个「看不出错」的缺陷，只有在类型和测试里留下形状，才不会有人再写回去。
>
> ★ 顺带一条：**这个缺陷只能在走查里被发现**（`live` slice 引不到裸 Node 里）。
> 它被抓到不是运气 —— 那条断言是**逐字比对**的，而 45 与 66 在逐字比对下藏不住。

**七、自适应窗口的收手条件，要表达「你想观察的那个瞬间什么时候出现」，而不是「什么时候还在忙」。**

场景 3 要观察的是「切回来时它**还在跑**」—— 所以「还在产出」是**回来**的理由。
第一版写反了：它把「还在产出事件行」当成**继续待着**的理由（静默才收手），
于是那个从头到尾都在产出的轮次把窗口拉到上限，回来时它已经跑完、
缓冲被终态推送丢掉，一条读数都没抢到。

> 一般化：一个「等条件满足」的循环，条件写错时**不会报错，只会一直等**（或一直不等）。
> 所以每个这样的循环都要问一句：**我等的那个瞬间，是这个条件成立的时候，还是它不成立的时候？**

### 8.8g M7a 实现纪律：六条

M7a 的产出有两半：**探针**（CLI 的 stdin 到底收什么）与**装配**（`context-builder`）。
前三条来自探针，后三条来自走查。★ 这一轮**没有产品缺陷** —— 它的价值全在
「**一句写在五个地方的断言，被一次真跑推翻了**」，以及为了让那次推翻站得住而设计的判据。

**一、一个从未实测过的断言，写在文档五个地方，仍然是断言。**

M7a 的整个装配方案压在 `design.md:1173` 的一句话上：「stdin 的 stream-json 输入本来就支持
多条消息，包括 assistant 角色的历史」。它被**五处**标着「未实测」，而那五处标记**不能**让它变真。
实测把它**推翻了**：`assistant` 与 `system` 两种内层 role **都被拒**
（`Error: Expected message role 'user', got 'assistant'`），3 条 user 行给出 **2 条** `result`。

> 规矩：**凡是「整个方案的形状」压在某个未实测事实上的，那个事实要有一个只测它的探针。**
> 而且探针的文件头第一句就要写「它测的是哪一句断言」——
> 否则探针跑了、数也记了，而没人知道它到底推翻/保住了什么。

**二、★ 负对照一次只许改一个变量 —— 否则那一臂的结论是「空白」，不是「否证」。**

A 臂同时改了两件事（**行数** 3 vs 1、**role** 是 `assistant`），而报错只指向 role。
于是「3 条 user 行会怎样」在 A 那里是**一片空白** —— 而**恰恰是那件事最危险**（见第三条）。
D 臂与 A 只差那**一个 role 字节**，正是它给出了这一轮唯一的关键否证。

> 规矩：**报错指向的变量与你想测的变量不是同一个时，这一臂什么都没测到。**
> 想隔离一个变量，就让它成为两臂之间**唯一**的差；做不到就如实写「这一臂判不了 X」。

**三、★★ 「被接受」的失败比「被拒绝」的失败危险得多 —— 断言要按最坏的那个设计。**

多条 stdin 行的失败模式**不是被拒**，是**被当成 N 轮**。而现成代码会把它**掩盖**：
`claude-adapter` 消费到第一条 `done` 就 `closeStdinOnResult`（默认 `true`）**在第一行上 `stdin.end()`**，
于是 result #2..N 被**我们自己**掐掉 —— 症状是「一轮用户消息在界面上变成一条被折叠的助手消息，
成本只记了 1/N」，一个安静的、看起来完全正常的错。

> 规矩：**问一句「它要是坏，最坏能坏成什么样」，然后按那个样子设计断言。**
> 这里最坏的样子在输出侧**没有回声**（CLI 不回显我们写进 stdin 的 user 行），
> 所以那句「数归档里 `type:"result"` 的行数」才是这一层价值最高的断言。

**四、★ 判据不许自己骗自己：它必须锚在只可能来自被测物的串上。**

走查的头号验收是「第 2 轮记得第 1 轮」，而**大部分判据都做不到这一点** ——
「第 2 轮提到了第 1 轮的问题」这种事，模型完全可以顺着第 2 轮自己的提示词编出来。
最终用的是：第一轮**回答里模型自己编的 4 位数**（本次 `7314`），
并且**两轮的提示词都查过、里面都没有它** —— 于是它可以被算出来，但**不可能被抄到**。

> 规矩：写完一条判据先问「**这个串还可能从哪儿来？**」
> 答案不是「只有那一处」就换一个；换不到就**把这条判成 `unknown`**，别判成 ✅。

**五、行为判据要带前提；前提不成立时降级成 `unknown`，不许照印 ✅。**

「cwd 的 `CLAUDE.md` 只进一遍」里 CLI 那一半靠**模型逐字复述标记**来判，
而它成立的前提是「**这一轮没用过任何工具**」—— 模型一旦自己 `Read` 过 `CLAUDE.md`，
复述就毫无信息量（读到的内容与自动注入的内容长得一模一样）。
所以报告把 `tool_start = 0` 当**前提**查，前提不成立时那两条判**降级成 `unknown`**。
同理：**阳性对照**（我们注入的那份）没成立时，所有「没被注入」的结论**全都不可信** ——
这一点要由报告**说出来**，不能让一片 ✅ 自己站着（§8.8e 规则三）。

**六、注释里一个 `*/` 会提前闭合块注释，而 `tsc` 的报错落在二十行之外。**

文件头想写归档通配路径时，写成「`16-48-*` 与 `16-51-*`」，**不写那个两字符序列**。
与 §8.8f 规则一是同一族：**`tsc` 不看注释和模板字符串里面的内容**，
所以那里的错永远出现在一个看起来完全正常的地方。

### 8.8h M7b 实现纪律：六条

M7b 这一轮的六条**全部来自走查脚本与报告**，产品代码（`src/`）在这一轮**一个字节都没改**：
第二次真跑报了三条 ❌，全是**采集与判据**的错；第三次真跑全绿。这正是 §7.3 那条顺序纪律
（「一个坏掉的采集器会把真钱花在**采集一个错的报告**上」）的又一次现身 ——
只是这一次，坏掉的不是采集**通道**，而是采集的**判据**。

**一、凭「最近/第一个」认出来的 id，是假证据，不许进档案。**

`waitTurnId` 取的是「`since` 之后**第一个**带 `turnId` 的帧」—— 对第 2 次发送，
它返回的是**另一个成员**（Nyx）的轮次 id，而那个 id 被当成「Atlas 这一轮」写进了归档。
**归档里因此有一条写错的证据**，而报告照样往下走。这不是小瑕疵：归档是这一里程碑
唯一的产出物，**一条假 id 会让「这一跳干了什么」整段判错**。
修法是删掉它，换成 `waitOwnTurnId`：从 `runtime:getState` 的活轮次里认，
判据钉死在产品自己的结构事实上（**发言者会话 + `hopDepth === 0` + 没被别的调用认领过**），
认不出就**如实说等不到**（`return null`），并且把降级路（从帧里认）**在记录里标出来**
（`via: 'batch-fallback'`），让读档案的人知道这一条有多可信。

> 规矩：**一个凭「最近」「第一个」「大概就是它」认出来的标识符，不许写进归档。**
> 认它的依据必须是产品的某个结构事实（谁、多深、什么状态），而且降级路径要在档案里留痕。

**二、走查脚本的缺陷会伪装成产品缺陷 —— 先查字节，再下结论。**

`composeAndSend` 把 `cc` 也拼进了正文，于是第二次发送的正文比第一次多 6 个字节（`@Echo `）。
后果：去重键不同 ⇒ Juno 收到 2 条转述、2 条轮次，`mention-deduped` 一次都没出现 ——
报告里三条 ❌，**而产品完全是对的**。
发现它的方式是那句**逐字比对**的断言（48 字 vs 54 字）—— 数字藏不住。

> 规矩：**走查每个动作都要有「能被逐字比对」的那一面**（正文长度、逐字相同、字节数）。
> 出现差 6 个字节这种量级的偏差，**先怀疑脚本**，别先怀疑产品。

**三、按「会话」扫出来的数字，至少混着三样东西。**

同一个会话里躺着：① 这条链的轮次；② **别的链**的轮次（协议在 `systemPrompt` 里、
对人设中立，任何空间都可能自发长链）；③ **转述消息** —— 它也是 `role: 'user'`，
只是作者是**派发方**而不是用户。第一版按会话扫，于是「Nyx 3 条 / Juno 2 条」这样的数字
与期望对不上，而派发本身一条不差。

> 规矩：**计数要顺着链路数，不要按容器扫。** 在本项目里就是：
> 「派发轮次的触发消息是**转述消息**」是个结构事实，所以数轮次要经由
> `触发消息 → 是哪条转述 → 谁派的`，而不是 `WHERE session_id = ?`。

**四、断言不许判「字面量」，要判「结构」。**

`!relayBody.includes('<mentions>')` 这个断言**永远会红**：转述正文里那句说明
（「你原文末尾的 `<mentions>` 那一行已由本应用取出」）**故意印着**这个字面量。
正确的判据是「不许出现**完整的块**」，另一条正向判据是「转述正文包含被剥掉块之后的原文」。

> 规矩：**断言的字面量必须与「我们自己的合法输出」不重叠** ——
> 判「块」不判「标签」、判「结构」不判「词」。写完先问：这句话我们自己会不会印出来？

**五、报告里带归属的话，归属必须是算出来的，不是写死的。**

报告末尾那句「其中**来自 Sable 的那些**警告说明……」是**无条件**打印的，
而第三次真跑的三条警告分别落在 `Sable@1跳`、`Rill@1跳`、`Rill@3跳` ——
**话比证据宽**。改成从 `notice.payload.detail.turnId` 反查轮次、再取它的
`workspace_id` / `hop_depth`，把「本条落在几跳上」算出来再印；
并且把统计**收窄到熔断那一条**（`message` 里含「协作链」），否则
`mention-self` 这类**与乒乓无关**的 notice 会被算进「空转警告」里。

> 规矩：**一句带归因的话，要么把归因算出来，要么不说。**
> 报告里的每一句「这说明……」都要能指回一条具体的证据行。

**六、前提被推翻的检查要删掉，不许改成 `✅` 混过去。**

旧检查「Sable 里没有出现 2 跳以上的轮次」写的是一个**错误前提**（它只在第一次真跑成立）。
第二次真跑里 Sable 自己长出了 `Nyx@2 → Atlas@2`。这一条**不能**改成反向断言
（「Sable 里一定有 2 跳以上的轮次」同样是**在赌一个观察**），
正确处置是把它降级成**发现**（`say` 一行如实打印观察到的最大跳数与它属于哪个空间），
并且把「为什么会出现」写进文档（协议在 `systemPrompt` 里、对人设中立 ⇒ §4.5b-1 后果 2）。

> 规矩：**检查项承载的是判决，发现承载的是观察。** 前提一旦被推翻，
> 那条检查就不该继续以「判决」的身份活着 —— 它要么有一个**依据设计**的新前提，要么降级成发现。

### 8.8i M7c 实现纪律：六条

这一轮的产品代码**一次都没被走查抓到错**（真跑一次全绿），所以下面六条里有两条来自
**走查脚本自己踩的坑**、四条来自**「错了不报错」的形状在写之前就被挡住**的地方。
写下来的理由与 §8.8f 规则六相同：**一个看不出错的地方，只有在文档和测试里留下形状，
才不会有人再写回去。**

**一、★ 编排钩子绝不许抛 —— 因为它坏掉之后没有任何信号。**

`compaction.onTurnFinished` 跑在 `runner.run()` 的**同步尾段**（与 `fanout` 同一个缝）。
抛出去会变成调度器的一条 `run-crashed` warn，**而压缩从此再也不会发生** ——
安静的、永不触发的坏法。所以整个函数体是**一个 `try/catch`**，任何异常只降级成一条 warn。

> 这个形状特别危险的原因是：**「不压缩」本来就是正常状态**。
> 一个坏掉的压缩与一个「还没到阈值」的压缩，在库里、界面上、日志里**长得一模一样**。
> ⇒ 规矩：凡「坏掉后的行为与正常行为重合」的钩子，都要从结构上保证它**不可能抛**
> （而不是靠「小心点写」），并且把「它抛了」这件事**单独留一条 warn**。

**二、★ 一个纯函数可以回显它不拥有的东西，但不许重算它。**

`shouldCompact` 返回的 `throughSeq` 只能是**传进来的**那个值（通过 = `lastBeforeTriggerSeq`，
否决 = 现有水位线）。★ 区间右端的**所有者是那条 SQL**（`seq < ?` 是「不含本轮触发消息」的
唯一实现）。纯函数若自己算 `triggerSeq - 1`，它会**看起来完全一样** ——
直到有一天出现软删除的行或 `seq` 缺口，它就会悄悄压掉一条**不该压的**。

> 规矩：凡「这个区间是谁的」有唯一答案的地方，纯逻辑层只能**回显**。
> 落地形态是一条用例的名字：「`throughSeq` **等于传入值**，不是重算」。

**三、★ 「丢失可以发生，但必须自己说出来」—— 声明要写进产物本身，不是写进日志。**

摘要超过上限时**从中间删、保留首尾**（丢掉最旧或最新的整段，比丢掉中间更伤），
并在删除处插一行 **in-band 声明**（`…（此处因摘要上限省略了 seq a–b 的 m 条骨架 ——
原文不在你的上下文里，且本摘要也不再完整覆盖它）…`）；**声明行永远不许被删** ——
上限小到荒谬时返回「标题 + 声明」，而不是「标题」。

> 一般化：**任何有损的变换，都要把「我丢了什么」写进它自己的产物里。**
> 写进日志不算 —— **模型读不到日志**。（原文行仍在库里：压缩只写 `inject_mode`，不删数据，
> 所以这条丢失是**模型视角**的，不是数据视角的。）

**四、★ 调试目标一出现就往页面里写东西，可能写在一个尚未提交的文档上。**

症状**不是**「注入失败」，而是**两三句之后在别处炸成 `undefined`** ——
因为采集器被打进了一个**即将被加载覆盖**的文档。★ 而 `page.url` 那一刻**已经是**
`file:///…/index.html`：**URL 不是判别依据**。

> 修法是两步，缺一不可：**先读一个只可能由真文档提供的证据**
> （`readyState === 'complete'` ∧ `window.api?.invoke` 是函数），
> 再注入并**回读验证**（注入 — 检查 — 重试三次）；重载则用「**旧文档的戳没了**」当判据。
> 规矩：**「目标出现了」不等于「目标已经是你以为的那个文档」。**

**五、★ 等待的顺序是「认领到这一轮 → 等终态 → 等安静」，反了会得到一个骗人的库转储。**

`waitQuiet`（「连续几次轮询都没有活轮次」）**单独用不可靠**：点完发送之后，
连续三次轮询都可能读到 0 条活轮次 —— 那一轮**还没排进** `runtime:getState`。
那样得到的转储**看起来只是「这一轮还没压」**，而真相是「还没开始跑」。

> 所以先用**库事实**把这一轮锚住（发言者会话 + `hopDepth === 0` + 没被别的调用认领过），
> 再等终态，最后才等安静；**认不到就如实说等不到**并降级（记录 `via`）。
> 与 §8.8h 规则一同族：**凭「最近/第一个」认出来的 id 是假证据**；
> 这里是「凭『没有活轮次』判出来的安静」是**假安静**。

**六、按容器扫出来的数字，前提要先被一个锚点钉住。**

这一跑的所有判据都建立在「下面那 3 条就是本走查发的那 3 条」上，
而那个前提**不是自动成立的**（沙箱里还可能躺着夹具阶段留下的轮次、别的链的轮次）。
所以报告里有一条「**夹具阶段的轮次数 = 0**」的检查 ——
它是那个前提的锚点，锚点掉了，后面按会话扫出来的每个数字都要重判。

> 规矩：**先钉住「容器里只有我要的东西」，再按容器扫。**
> （§8.8h 规则三是它的反面形态：那里容器里**混着**别的东西，于是要顺着链路数。）
...
13. ~~**压缩事件：`system:compact_boundary` 若真出现，是 bug 还是一类事件？**~~
    ✅ **M7c 收口（2026-09-24）：它是一类事件，而且**我们这边**的判决是「把它当 bug 对待」。**
    落地形状是 `interpretCompactResult`（`domain/compaction-service.ts`）——
    一个**纯函数**，把「CLI 说了什么」与「我们判该不该压」合成**一个要不要露面的判决**，
    四条规则（就是这张表）：

    | CLI 报告了 boundary | 我们判该压 | CLI 的 `compact_result` | 露面 | 理由 |
    |---|---|---|---|---|
    | 真 | 任意 | 任意 | **warning** `compact-boundary-seen` | §5.3 的「出现即算错窗口」。**在实现上它只能是可见的** —— 一个只进日志的 bug 与一个没发生的 bug 长得一样 |
    | 假 | **真** | `failed` | **warning** `compact-cli-failed` | 我们要压、CLI 说它压失败了 ⇒ 上下文**即将失控**，这是用户该知道的事 |
    | 假 | 假 | `failed` | 不露面，只进日志 | `too_few_groups` = 没东西可压 = **正常**（M5 就观测到过这一条） |
    | 假 | 任意 | 其它 / 无 | 不露面 | 无话可说 |

    ★ 判据是**两个事实的合取**（CLI 的信号 + 我们自己的判决），单看任何一个都会误报：
    只看 CLI 的话，「没东西可压」这个**正常**结果会天天弹警告；只看我们自己的判决，
    就永远发现不了「我们和 CLI 对同一份历史有不同意见」这件事。

    ⚠️ **反面证据缺失，如实记**：M7c 的真机走查里 `compact_boundary` / `compact_result`
    **一次都没出现**（三轮，我们在阈值 2 条上永远先压）。所以这张表在真机上**只验证了
    「不露面」那一半**（第一、二行一次都没被触发过），它们的**触发路径只有单测**。
    见 §14.3。

**M5 带出来的待办**：

14. ★ ~~**`system:status` 带 `compact_result: failed` 时，要不要让用户看见？**~~
    ✅ **M7c 收口：要，但**只在它真的意味着失控时**。** 判据见上面 13 的那张表第二行——
    答案是「同一条 `failed`，要分两种读法」：`too_few_groups` 是**正常**（没东西可压），
    与「我们判该压」合起来才是异常。M5 当时把它记成一条 `warn` 诊断（§2.3-5），M7c 保留了
    「**永远进日志**」（`compaction:cli-signal`），只把**合取成立**的那一种升格成 `app:notice`。
15. ★ ~~**`cwd/CLAUDE.md` 的重复注入**（§8.5c-1）。~~ ✅ **M7a 收口（2026-09-23）**：
    注入的那一份**剔掉 `CLAUDE.md` 与 `.claude/CLAUDE.md` 两个路径**，其余四个照常注入
    （依据与代价不对称的论证见 §8.5c-1 的 M7a 块）。
...
19. ~~**`TurnContext.messages` 的多条消息形态尚未实测**（§4.4e）。~~ ✅ **M7a 收口（2026-09-23）**：
    `scripts/m7a-probe.ts` 的四臂实测给了两个答案，**都是否定的**，而且第二条改变了 §4.6 的形状：
    - **内层 `message.role` 只能是 `'user'`**：写 `assistant`/`system` ⇒ CLI 在**解析 stdin 阶段
      就把整轮拒掉**（stderr 逐字 `Expected message role 'user', got 'assistant'`，stdout **0 行**）。
    - **多条 user 行不是「一轮的多条消息」**：3 行 → **2 条 `result`**，切在 `1 | {2,3}`
      （第 1 轮的模型只看得见第 1 行，第 2 轮看得见全部三条且 `cache_read_input_tokens ≈ 21k`
      —— 它接着第 1 轮的会话在跑）。⇒ 那是**多轮对话**协议：**发 N 行 = 发 N 轮**。
    ⇒ 结论落在 §4.4e 与 §4.6：**数组留在层间，拍平成恰好一行是硬要求**（不再是「保守起见」）。

**2026-09-24 M7b 真机走查带出来的待办**：

20. **空转计数要不要把 0 跳（用户那一跳）算进去？** 现在算。后果是**一次纯问答的 `@`
    也会看到「协作链没有实际动作」的警告**（见 §4.5b-1 后果 1 的实测形状：
    `Vex@0` 与 `Wisp@1` 都没碰工具 ⇒ `emptyHops = 2` ⇒ 警告）。
    ★ 反面同样实测到了：**含 0 跳会让「有没有警告」取决于收尾顺序** ——
    同一次真跑里 Sable 那条链每一跳都没干活（全归档工具事件数 0），却一条警告都没出，
    因为它的兄弟跳与还在跑的 0 跳把后缀切断了（§4.5b-1 后果 1）。
    两种口径都说得通，**而现在这个口径有一个明确的代价**：
    - **含 0 跳（现状）**：警告口径 = 「这条链到目前为止一点动作都没有」。
      用户发一条 `@A` 的问题、A 只是回答（没碰工具），就会看到一条警告 ——
      它说的是**真话**，但**可能构成噪音**。
    - **不含 0 跳**：警告口径 = 「派发出去之后连续两跳没有动作」，
      干净得多；代价是**用户自己那一跳不算数**，于是一条「用户不停地 @、每个成员都只回话」
      的链要比现在多一跳才会被警告。
    ★ 这是**阈值的定义问题，不是实现缺陷**，所以不在 M7b 里改。
    要判它，需要先有「警告噪声率」这个观察 —— 走查的归档里已经能数出来
    （每条警告落在几跳上、`emptyHops` 是多少），**先攒样本再拍板**。
    ⚠️ 无论选哪个，**四个阈值数字（2 / 4）本身不要顺手改** —— 它们是 §4.5b 里定的。

**2026-09-24 M7c 真机走查带出来的待办**：

21. ★ **我们自己的折叠说明会被再折一次。** 那条 `role:'system'` 的说明行
    （`【系统】历史上下文已压缩：seq ≤ 2 的 2 条消息已折叠成摘要…`）本身也是 `message` 里的一行，
    于是**下一次压缩把它当历史折进摘要**，得到：
    `[5] 【系统】 【系统】历史上下文已压缩：seq ≤ 2 …` —— 标签重了一遍，
    而那行内文里的条数是**当时**的读数（与摘要标题里的条数并排读会像互相矛盾）。
    **实测形状在归档里**（`scripts/evidence/m7c-2026-09-24T16-20-01-501Z`，`db-A-final.json` 的
    `rolling_summary`）。**M7c 明知而不改**，理由写在 `m7c-walkthrough.ts` 的报告里，两条：
    ① 它不撒谎（没有假声明，原文仍在库里），也不多泄露（那一行本来就作为系统消息进下一轮上下文）；
    ② 改它要么动 `listBySessionBetween` 的谓词、要么给这一行一个专门的标记，**两者都要重跑真机**，
    而读数已经印成结论 —— 在里程碑最后一步换掉一条已印的读数不划算。
    ⚠️ 真要改的话，**注意别用「凡是 `role='system'` 就不折」**：接力（`@`）的系统行是**真内容**，
    剔掉它会丢东西。可行的两条窄路：给折叠说明一个可判别的标记（比如它没有 `turn_id`），
    或者让说明行**不进摘要的骨架集**而只作 UI 提示。
22. **那两个阈值常量仍然是估算**（`compactAtChars = 400 000` / `COMPACT_READ_LIMIT = 500`）。
    它们**各自都有单测**（字符硬闸单独触发、撞读取上限 → warn 且水位线只推到读到的最后一条），
    但**真机上一轮都没碰到**：三轮里历史最长 6 条、正文最多几百字。
    要判它们合不合适，得有一个**真的长会话**（几十轮 + 大正文），那是 M8/M9 之后的事。
    ⚠️ 在此之前**不要**因为「看起来太大」就顺手调小 —— §5.3a 的教训是同形的：
    一个在未知数字上的改动，会把「没测过」变成「测错了」。
23. **走查的 `[runtime:…]` 归档是「按 tag + 行首一段去重」的摘要**，同一 tag 的多条读数
    （比如每轮一条的 `compaction:decided`）只留第一条 —— 逐字原文在 `app-A.stderr.log` 里。
    这不是产品缺陷（是采集器的取舍），但它有一个真实的坑：**报告若想知道「每一轮各判了什么」，
    必须去读那份原始日志**。M7c 的报告已经把它写在自己的输出里，免得下一个人在这上面栽一次。
