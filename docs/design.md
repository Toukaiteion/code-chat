...
| **M7a** | ✅ **已落地**（2026-09-23，commit 未做）`context-builder` + 消息数组化 + `systemPrompt` + **两件未实测事实的探针**。 | 第 2 轮能正确引用第 1 轮；cwd 的 `CLAUDE.md` **只进一遍**；★ `cache_read_input_tokens` **在本机这个第三方端点上不能用来验收前缀缓存**（自定义前缀在新进程里恒为 0，只有**同进程续写**才非零 —— 见 §8.9-16 与归档 `scripts/evidence/m7a-2026-09-23T16-48-08-427Z` / `…16-51-08-216Z`），所以这条验收要按它的端点事实读，**别拿 M6b 的 37248 当基线**。**逐条读数与结论与它的文档收口一起写**（§六下方的实证小节同理） |
| **M7b** | ✅ **已落地**（2026-09-24，commit 未做）`mention-service.ts` + `@` 派发 + 跳数记账 + **§4.5b 的三条熔断**。 | 见下方「M7b 实证」：① 多 `@` 广播 ② agent→agent 派发（标记块）③ 2 跳空转 → 可见警告 + 落库 system 事件 ④ 4 跳 → 链被终止、扇出被取消 ⑤ 去重与 `cc` ⑥ 并发 ≤ 3 |
| **M7c**（下一个） | **压缩**：`compaction-service.ts` + 阈值 + 确定性摘要 + 落库 + `inject_mode` 的消费。 | 强制触发阈值（注入小 N）→ 下一轮装配里 `<summary>` 在位、被压的消息不在数组里；`compacted_through_seq` 非零、`rolling_summary` 非空；记一次「N 条 ≈ 多少 token」的**标定映射**；`compact_result` 若出现，断言它是否露面及理由 |
...
⚠️ **M7a 的探针与走查**（`probe:m7a` / `walk:m7a`）的命令与场景**已按计划落进 `package.json` 与脚本**，
但它的**结论与逐条读数**（多条 stdin 是否被接受、cwd 的 `CLAUDE.md` 是否只进一遍、
`cacheRead` 在那个端点上的实际形态）**与 M7a 的文档收口一起写** ——
这一段的位置说明那个结论还没落定。
★ 已确定的一条**会误导后来人**的端点事实先记在这里：本机这个第三方端点的 `cacheRead`
**不能**用来验收前缀缓存（自定义前缀在**新进程**里恒为 0，只有**同进程续写**才非零），
证据在 `scripts/evidence/m7a-2026-09-23T16-48-08-427Z` 与 `…16-51-08-216Z`；见 §8.9-16。
...
> **（§8.8g 是 M7a 的，与它的文档收口一起写 —— 先占号，免得后面的里程碑互相顶号；
> 这一段的位置本身就说明 M7a 的纪律还没落定，别把它读成「M7a 没踩过坑」。）**
