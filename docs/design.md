...
10. **启动期的 `turn.reapOrphans()` 尚未接线**（§4.4）。`turn-repo.reapOrphans` 已就位、已有用例，但 `src/main/index.ts` 刻意没调它 —— 孤儿清扫连同 `taskkill` 记录的 PID 是 **M10** 的整块工作，M3 不做以免里程碑边界模糊。**在那之前，硬杀应用会留下 `status='running'` 的僵尸轮次**，这是已知且已接受的中间状态。
